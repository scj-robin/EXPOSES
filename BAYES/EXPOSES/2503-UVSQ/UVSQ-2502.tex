...
\frame{\frametitle{Latent variable models in ecology}
  ...
  \begin{itemize}
    ...
    \item encode dependency (due to space, relatedness, ...), 
...
\frame{\frametitle{Poisson log-normal model}
  ...
  \begin{tabular}{ccc}
    ...
    \includegraphics[width=.25\textwidth, trim=20 10 20 10, clip=]{\figeco/BarentsFish-coeffAll-woIntercept} & 
    \includegraphics[width=.25\textwidth, trim=20 10 20 10, clip=]{\figeco/BarentsFish-corrPred} & 
    \includegraphics[width=.25\textwidth, trim=20 10 20 10, clip=]{\figeco/BarentsFish-corrAll}
  \end{tabular}
  $$
}
...
\frame{\frametitle{EM algorithm}
  ...
  \bigskip \bigskip \pause
  \paragraph{Problem.} 
  For many models (including PLN and SBM), $p_\theta(Z \mid Y)$ is itself intractable 
}
    ...
    \begin{tabular}{p{.6\textwidth}}
      \onslide+<1->{\paragraph{Distribution path:}  
        ...
        \begin{array}{rlrl}
          ...
          & = \frac{\pi(\theta, Z) \ell(Y | \theta, Z)}{p_0(\theta, Z)}
        \end{array}
        $$}
    \end{tabular}
    & 
