...
\date[Mar.'25, UVSQ]{
  ...
  ~ \\
  Univ. Versaille - Saint Quentin, Mar.'25
  }
...
\frame{\frametitle{Example 2: Species interaction networks}
  ...
  \begin{itemize}
    ...
    \item number of shared parasites (unipartite, unoriented, weighted), 
    \item ....
  \end{itemize}
  ...
  $$
  Y = (Y_{ij})_{i, j = 1 \dots n} \sim \Fcal(\cdot; (x_i)_{i = 1 \dots n}, (x_{ij})_{i, j = 1 \dots n}, \emphase{\theta}).
  $$
...
\frame{\frametitle{Stochastic block-model}
  ...
  \begin{itemize}
    \setlength{\itemsep}{.5\baselineskip}
    \item $\emphase{\pi_q} =$ proportion of species belonging to group $k$ (\emphase{\sl node clustering})
    \item $\emphase{\alpha_{q\ell}} =$ group effect on the interaction intensity
    \item $\emphase{\beta} =$ effect of the covariates on the interaction intensity
  \end{itemize}
...
\frame{\frametitle{Example 2: Variational EM (VEM) for the stochastic block-model}
  ...
  \begin{itemize}
    ...
    \item Approximate membership probability $\widetilde{\tau}_{ik} = \Pr_{\qt}\{Z_i = k\}$, 
    \item Lower bound $ELBO(\widehat{\theta}, \qt)$ (R packages \url{blockmodels} \refer{Leg16}, \url{sbm} \refer{BDL17})
  \end{itemize}
  }
}
...
\frame{\frametitle{Discussion}
  ...
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item Monte Carlo approaches as still \emphase{computationally demanding}
    \item \emphase{ML-based variational inference} (e.g. variational auto-encoders, normalizing flows) may correspond to larger approximation class $\Qcal$, therefore to more flexible approximate distribution $\qt$, getting closer to genuine MLE
    \item More insights about the \emphase{theoretical properties of variational estimates} would be more than welcome \refer{WeM19,TaT21,CGG24,GaL23}
  \end{itemize}
