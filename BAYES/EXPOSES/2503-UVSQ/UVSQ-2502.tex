...
\frame{\frametitle{EM algorithm}
  ...
  \begin{itemize}
    ...
    $$
  \end{itemize}
  $\to$ The log-likelihood $\log p_\theta(Y)$ increases at each step.
  ...
  \bigskip \bigskip \pause
  \paragraph{Problem:} This distribution is itself intractable for many models, including PLN and SBM
}
...
\frame{\frametitle{Variational EM (VEM) algorithm}
  ...
  \smallskip
  $\to$ No measure of uncertainty (no confidence interval)
...
\frame{\frametitle{Variational EM (VEM) for the stochastic block-model}
  ...
  \begin{align*}
    ...
    & \text{Pair $(i, j)$:} &
    Y_{ij} \mid Z_i = q, Z_j = \ell & \sim \Pcal(e^{\alpha_{q\ell} + x_{ij}^\top \beta})
...
%====================================================================
\frame{\frametitle{Toward genuine Bayesian inference}
  ...
  \end{align*}
  $\to$ Aim of the inference: be able to sample from the joint \emphase{posterior $p(\theta, Z \mid Y)$}
  
  \bigskip \bigskip
  \paragraph{Prior:} $p(\theta) = \Dcal(\pi; a) \times \Ncal([\alpha \; \beta]; m, V)$. 
  
  \bigskip 
  \begin{tabular}{cc}
    \hspace{-.04\textwidth}
    \begin{tabular}{p{.6\textwidth}}
      \onslide+<2->{\paragraph{Sequential Monte Carlo sampling.} \refer{DDJ06}}
      \begin{itemize}
      \onslide+<2->{\item $\textcolor{red}{p_0} = $ proposal, $\textcolor{blue}{p^*} = $ target posterior \\ ~ \\}
      \onslide+<3->{\item Intermediate distributions
      $$
      \textcolor{red}{p_0}, \; p_1, \; \dots, \; p_H = \textcolor{blue}{p^*}
      $$}
      \onslide+<3->{\item Iteratively: use $p_h$ to get a sample from $p_{h+1}$}
      \end{itemize}
    \end{tabular}
    & 
    \hspace{-.1\textwidth}
...
\frame{\frametitle{Proposed sequential sampling strategy (1/2)}
  ...
  \paragraph{Distribution path:} \refer{DoR21} 
    set $0 = \rho_0 < \rho_1 < \dots < \rho_{H-1} < \rho_H = 1$,
  \begin{align*}
     p_h(\theta, Z) & \propto \pt(\theta, Z)^{\emphase{{1-\rho_h}}} \; \times \; p(\theta, Z | Y)^{\emphase{{\rho_h}}} \\
%      \\
     & \propto \pt(\theta, Z) \; \times \; r(\theta, Z)^{\emphase{{\rho_h}}}, 
     & r(\theta, Z) & = \frac{\pi(\theta, Z) \ell(Y | \theta, Z)}{\pt(\theta, Z)}
  \end{align*}
  ...
  $$
  $\rho_h$ can be tuned so to guaranty a sufficiently high effective sampling size (ESS).
...
\frame{\frametitle{Proposed sequential sampling strategy (2/2)}
  ...
  \paragraph{Initial proposal distribution $p_0$:} based on VEM outputs:
  $$
  \pt_0(\theta) =  \Dcal(\pi; \widetilde{a}_0) \times \Ncal([\alpha \; \beta]; \widetilde{m}_0, \widetilde{V}_0)
  $$
  with  $\widetilde{a}_0 = f(a_0, \widetilde{\tau}_{VEM})$, $\widetilde{m}_0 = f(m_0, \widehat{\theta}_{VEM})$ and $\widetilde{V}_0 = f(S_0, - \nabla_\theta^2 ELBO(\widehat{\theta}_{VEM}, \qt_{VEM}))$.
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{c}
      \hspace{-.05\textwidth}    
      \includegraphics[width=.4\textwidth, trim=5 0 5 00, clip=]{\figbayes/DoR21-Fig1-simu-rho} 
    \end{tabular}
  \end{tabular}
...
\frame{\frametitle{Tree interaction network}
  ...
  \paragraph{Posterior distribution of $\beta$.} For $\widehat{K} = 4 = \argmax_K p(K \mid Y)$, in 26 SMC steps:,
  $$
  \begin{tabular}{ccc}
...
\frame{\frametitle{Variational EM (VEM) for the Poisson log-normal model}
  ...
  \bigskip \bigskip \pause
  \paragraph{No EM.} 
  The $Z_i$'s are marginally Gaussian, but not Gaussian conditional on the $Y_{ij}$'s \\
  (no close form, even in one dimension)
  
  \bigskip \bigskip 
  \paragraph{Variational EM.} Gaussian approximation \refer{CMR18a}, \url{PLNmodels} R package
  $$
  ...
  \item Lower bound $ELBO(\widehat{\theta}, \widetilde{m}, \widetilde{S})$
  \end{itemize}
  
}

%====================================================================
\frame{\frametitle{Monte Carlo EM}

  \paragraph{Monte Carlo EM.} \refer{CeD85}
  \begin{itemize}
    \item Monte Carlo E step: Sample $(Z^m)_{m = 1 \dots M} \overset{iid}{\sim} p_{\theta^{(h)}}(Z \mid Y)$, then evaluate
    $$
...
%====================================================================
\frame{\frametitle{Composite log-likelihood}

  \refer{StR24} \refer{VRF11}
...
\frame{\frametitle{Discussion}
  ...
  \paragraph{Summary.}
  \begin{itemize}
    \item Latent variable models are widely in statistical ecology
  ...
  \begin{itemize}
    ...
    \item ML-based variational inference (variational auto-encoders) may correspond to larger approximation class $\Qcal$, therefore to flexible approximate distribution $\qt$, getting closer to genuine MLE
    \item Better understanding the properties of variational estimates
  \end{itemize}
