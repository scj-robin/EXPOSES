...
\paragraph{Sidak:} 
If tests are independent, we have
$$
FWER = 1 - \Pr\{FP = 0\} = 1 - (1-\alpha)^{n_0}.
$$
that insures $FWER = \alpha^*$ if each test is performed at level
$$
\alpha = 1 - (1-\alpha^*)^{1/n_0} \quad (\simeq \alpha^* / n_0)...
$$
.. but the genes expressions (the tests, consequently) are \textblue{not
  independent}.

\paragraph{Bonferroni correction}
is based on the inequality
$$
\Pr\left\{\bigcup_i A_i\right\} \leq \sum_i \Pr\{A_i\} = n_0 \alpha
\leq n \alpha
$$
and implies that performing each test at level $\alpha^*/n$ insures \\
...
replacing $n$ by $n-1$ 
\item[$\vdots$]
\item[$k.$] Apply the same correction to $P_{(k)}$, replacing $n$ by
  $n-k+1$.
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\paragraph{Adjusted $p$-values} are $p$ values that can be 
...
\item one step Bonferroni
  ...
  \tilde{P}_i = \min(n P_i, 1) \leq \alpha^*
  $$
\item  one step Sidak 
  $$
  P_i \leq 1 - (1 - \alpha^*)^{1/n}
  \qquad \Longleftrightarrow \qquad 
  \tilde{P}_i = 1-(1-P_i)^n  \leq \alpha^*
  $$
\item adaptive Bonferroni \qquad (Holm (79))
  $$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage 
\paragraph{Crucial point:}
The $p$-values of null genes are uniformly distributed: $i$ null
$\Rightarrow P_i \sim \Ucal[0, 1]$.
$$
\begin{tabular}{ccc}
  like this: & not like this: & nor like this: \\
...
\paragraph{Remarks:} 
\begin{itemize}
\item Estimating $n_0$ improves the procedure.
\item The Benjamini \& Yekutieli (01) criterion: \qquad
  $P_{(i)} \leq i \alpha^* \left/ \left( n \sum_j 1/j\right) \right.$ \\
...
$$
\tilde{P}_{(i)} = \min_{j \geq i}\{\min[n P_{(j)} / j, 1]\}.
$$
...
a mixture model.  
$$
T_i \sim f(t) = \pi_1 f_1(t) + \pi_0 f_0(t)
$$
...
\begin{tabular}{ll}
  ...
  \\
  \paragraph{$f$:} & the alternative density \\
\end{tabular}
$$
...
\paragraph{Adaptive estimation of $t$.}
\begin{itemize}
\item Determine an optimal value of $t$ to estimate $a$.
\item Try to get an aestimate robust to contamination of the null
  distribution.
\end{itemize}

\paragraph{Accounting for dependecy.}
\begin{itemize}
\item Adapt the method to dependent tests, when the dependency
  structure is known (e.g. spatial structure).
\item Apply this to new kinds of microarray: CGH array, ChIP-chip.
\end{itemize}
