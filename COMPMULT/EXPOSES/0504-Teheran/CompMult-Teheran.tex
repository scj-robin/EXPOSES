...
\begin{center}
  ...
  \bigskip
   ...
   \vspace{2cm}
   {\Large Semi-parametric modeling} 
...
$\Longrightarrow \qquad$ they are too conservative.
...
$$
\begin{description}
\item[Step 1:] Apply (say) the Bonferroni correction to $P_{(1)}$: if
  $P_{(1)} \leq \alpha^* / n$ then go to step 2
...
\paragraph{Local FDR ($\lFDR$).} Initially defined by Efron \&
...
The efficiency of all multiple testing procedures would be improved if
$n_0$ was known.
...
$$
x > t: \quad G(p) \simeq (1-a) p.
$$
Storey \& al, Genovese \& Wasserman (JRSS-B, 02) propose estimates
of $a$ based on this approximation:
$$
\widehat{a} = [1 - P(t)/n] / (1-t).
$$

\noindent$(1-a)$ can also be estimated by the coefficient of the linear
...
data), they using a local logistic regression to estimate the \paragraph{\sl
  local FDR} ($\lFDR$):
$$
...
\noindent $\Hbf_0$ (negative) $p$-values are not uniformly distributed over [0, 1]. The
non-parametric part is contaminated by this departure of the nu.
...
\centerline{$
  \begin{array}{cccc}
    ...
    \quad & \quad 1-\widehat{\tau}_{(i)} \quad \\ 
    \hline
    1\% & 4 & 2.8\;10^{-5} & 0.988 \\
    5\% & 142 & 3.1\;10^{-3} & 0.914 \\
    10\% & 296 & 1.3\;10^{-2} & 0.798 \\
  \end{array}
$}
