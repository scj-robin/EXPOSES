...
\frame{\frametitle{{P}oisson-log normal (PLN) distribution}
  ...
  \begin{align*}
   ...
   \Var(Y_{ij}) & = \lambda_j + \lambda_j^2 \big( e^{\sigma^2_j} - 1\big) 
   & (\text{over-dispersion}) \\
   \Cov(Y_{ij}, Y_{ik}) & = \lambda_j \lambda_k \big( e^{\sigma_{jk}} - 1\big)
   & (\text{same sign as $\sigma_{jk}$}) 
   \end{align*}
}
...
\frame{\frametitle{Intractable EM}
  ...
  \begin{itemize}
  \item PLN is an incomplete data model: Try EM:
  $$
  \log p_\theta(Y) = \Esp[\log p_\theta(Y, Z) \,|\, Y] + \Hcal[p_\theta(Z\,|\,Y)]
  $$ 
  \item  EM requires to evaluate (some moments of)
  $$
...
\frame{\frametitle{Probabilistic PCA}
  ...
  $$
  \ra Still bi-concave in $\theta = (\emphase{B}, \beta)$ and $(\Mt, \St)$
...
\frame{\frametitle{Problem}
  ...
  \bigskip \bigskip 
  \paragraph{Statistical interpretation:} infer the graphical model \refer{Lau96} of the $Y_i = (Y_{i1}, \dots Y_{ip})$, i.e. the graph $G$ such that
  $$
...
\frame{ \frametitle{Variational inference}
  ...
  \bigskip \bigskip \pause
  \ra Still bi-concave in $\theta = (\emphase{\Omega}, \beta)$ and $\pt = (\Mt, \St)$. Ex:
  $$
...
\frame{ \frametitle{Discussion}
  ...
  \begin{itemize}
   ...
   \item Confidence interval and tests for the regular PLN
  \end{itemize}
}

%====================================================================
\frame{\frametitle{Statistical properties of variational estimates}

\paragraph{General properties of V(B)EM inference.}
\begin{itemize}
 \item VEM stationary point $\neq$ log-likelihood stationary point
 \item VBEM posterior variance are too small
 \item Some consistency results, typically when $p(Z\,|\,Y)$ asymptotically belongs to $\Qcal$ (SBM, Bayesian logistic regression).
\end{itemize}

\bigskip \bigskip \pause
\paragraph{Using V(B)EM output as a starting point} for regular inference:
\begin{itemize}
 \item PLN: get maximum-(composite-)likelihood estimates starting from $\widetilde{\theta}_{VEM}$ (proposed internship) \\~
 \item SBM: get the true posterior $p(\theta\,|\,Y)$ starting from $\pt_{VBEM}(\theta)$ (on-going work with Sophie). \\~ 
\end{itemize}
\ra Hopefully: few iterations are needed
