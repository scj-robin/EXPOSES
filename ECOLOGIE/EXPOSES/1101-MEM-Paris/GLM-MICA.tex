...
\date[MEM, Feb 2011, Paris]{Métagénomique des écosystèmes microbiens,
  February 2011, Paris}
...
\frame{\frametitle{An example (cont'd)}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.45\textwidth}}      
      ...
      \begin{itemize}
      \item Mean response for  condition $\xbf$:
        $$
        \Esp(Y) = \mu(\xbf, \thetabf),
        $$
      \item Observed response at experiment $i$: 
        $$
        Y_i \sim \Fcal[\mu(\xbf_i, \thetabf)].
        $$
      \end{itemize}
      
      \Pause\paragraph{Answers:} Estimate $\widehat{\thetabf}$ or
      some function of it.
    \end{tabular}
  \end{tabular}
  }
...
\frame[containsverbatim]{\frametitle{An example (cont'd)}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
      ...
      \bigskip
      \paragraph{On column = one variable:} \\
...
\frame{\frametitle{Linear models: Regression, Analysis of variance,
  ...
  The \emphase{mean response} $\mu$ can be written as a scalar product
  $$
  \emphase{\mu(\xbf, \thetabf) = \xbf . \thetabf}.
  $$
  \Pause 
  \vspace{-.25cm}\hspace{-.5cm}$
  \begin{array}{llc}
    \text{Model} & \hspace{-.25cm} \xbf \text{ (medium $A$, dose $d=3$,
    ...
    \end{array}
    & \hspace{-.25cm} [\underset{medium}{\underbrace{1\;0}} \;
...
\frame{\frametitle{Contrasts of interest}
  ...
  \item \Pause Does the species have the same abundance in the two
    ...
    $$
    (\alpha_A + \gamma_{Ad_{\max}}) - (\alpha_B + \gamma_{Bd_{\max}})
    \overset{?}{=} 0 
    $$
...
\frame{\frametitle{Link function}
  ...
  \item The \emphase{link function does matter for the interpretation}
    of the results.  
  \item It is the price to pay to preserve to
...
\frame{\frametitle{Practical use of generalised linear models}
  ...
  \item The interpretation of parameter values depend on the link function.
  \end{itemize}
  }
...
\frame{\frametitle{Sharing information}
  ...
  \bigskip 
  \begin{description}
  \item[Normalisation.] \Pause E.g. $\delta^{\emphase{Species}}_{Prep}$ can be
    supposed to be \emphase{the same for all species} or
    \emphase{smoothed} (e.g. 'lowess' for microarray data). \\
    \medskip
  ...
  \item \paragraph{Baysian inference} typically encodes such a
...
\frame{\frametitle{NGS data} 
  ...
  \Pause \bigskip
  \paragraph{Bioinformatics (and statistics) and High Throughput
    Sequencing - 2011:} 
  $$
  \begin{tabular}{c}
    22 March 2010, Institut Pasteur: \\
    \url{www.lirmm.fr/~rivals/HTS-2011}
  \end{tabular}
  $$
  }
