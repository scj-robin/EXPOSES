...
%====================================================================
  ...
  \bigskip 
  \paragraph{Data \goto{sec:batCallsModels}\label{sec:batCalls}.} $Y_k =$ number of bat calls during the $k$-th time bin. 
  ...
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item The model parameter $\theta$ is identifiable from the joint distribution of $(Y_1, Y_2, Y_3)$. \\
...
\frame{\frametitle{Markovian representation (Markov switching case)} 
  ...
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item The model is a regular Hidden Markov Model (HMM) with graphical model \\ ~
    \begin{figure}
      \begin{centering}
...
\frame{ \frametitle{Simulation study (not shown)}
  ...
  \begin{itemize}
    ...
    \item AIC does, when enough signal ($\lambda$) and discretization ($N$) \\
...
%     \item Clear advantage wrt Poisson HMM.
  ...
  Take $N = 2 n$ and use AIC to choose $Q$.
...
\frame{\frametitle{Model 2: Bats calls sequences}} 
%====================================================================
...
\frame{\frametitle{Vigie-chiro project} 
  ...
  \begin{itemize}
    ...
    \item Restricted to sequences with at least 50 calls $\to$ 1555 time sequences. 
  \end{itemize}
  
  \bigskip \bigskip \pause
  \paragraph{Poisson vs Hawkes / Homogeneous vs HMM.} Best model according to AIC\footnote{BIC: Poisson = 153 (homo = 132, HMM = 21), Hawkes = 1402 (homo = 775, HMM = 627).}
  $$
...
\frame{ \frametitle{An example}
  ...
  \paragraph{Conditionally most probable states.} (MAP)
  $$
  \begin{tabular}{ccc}
...
\frame{ \frametitle{States and species}
  ...
  The number of bat species was also recorded each night in each site.
