...
\frame{\frametitle{Joint species distribution model} 
  ...
  \begin{itemize}
    ...
    $$
    $o_{ij} =$ given 'offset' term, accounting for the sampling effort;
...
\frame{\frametitle{An example}
  ...
  \paragraph{Importan aim of JSDM:} Distinguish between abiotic and biotic effects:
  $$
  \begin{tabular}{ccc}
    Covariate effects & 
    Correlation induced & 
...
\frame{\frametitle{Inference} 
  ...
  \begin{itemize}
    ...
    which turns out to be intractable for the PLN model.
  \end{itemize}
...
\frame{\frametitle{VEM for the Poisson log-normal model}
  ...
  \begin{itemize}
    ...
    \item Very few theoretical guaranties: no general properties as for maximum likelihood (consistency, asymptotic normality, etc.) \\
    \medskip 
...
\frame{\frametitle{Toward genuine maximum likelihood inference \refer{StR24}}
  ...
  \begin{itemize}
   \item Sample $(Z^m)_{m = 1 \dots M} \overset{iid}{\sim} q^{(h)}(Z)$, \emphase{$q^{(h)} =$ proposal},
...
\frame{\frametitle{Proposed composite likelihood algorithm}
  ...
  \bigskip
  \paragraph{Proposition: EM applies to composite likelihood \refer{StR24}.}
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Same problem as the construction of a incomplete balanced block design\footnote{Not always possible: e.g., need to have $p \mid Bk$ and $p(p-1) \mid Bk(k-1)$} \goto{back:blocksCL} \label{sec:blocksCL}
  \end{itemize}
...
\frame{\frametitle{Simulation study}
  ...
  \begin{itemize}
    ...
    \item Effect of the block size on the variance of the estimates (actually small \goto{back:varCL}) \label{sec:varCL}
  \end{itemize}
  ...
  \medskip
  VEM = pseudo Fisher information matrix based on the ELBO,  \\
...
\frame{\frametitle{Model 3: Fish species from the Barents sea}}
%====================================================================
...
\frame{\frametitle{Fish species in the Barents sea}
  ...
  \begin{tabular}{ccc}
    ...
    \includegraphics[width=.25\textwidth, trim=10 10 25 50, clip=]{\figStR/Barents\exampleParms-betaSignif-cem5} & 
    \includegraphics[width=.25\textwidth, trim=10 10 25 50, clip=]{\figStR/Barents\exampleParms-sigmaSignif-cem5} &
    \includegraphics[width=.25\textwidth, trim=10 10 25 50, clip=]{\figStR/Barents\exampleParms-corSigmaSignif-cem5}
  \end{tabular}  
  $$
}
