...
\frame{\frametitle{Summary} 
  ...
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item Dealing with latent variable yields specific technicalities
    \item Ranging from trivial to intractable
    \item Often model-dependant, requiring ad-hoc(?) developments
...
%     \setlength{\itemsep}{.75\baselineskip}
    ...
    \item (Asymptotic) normality does not hold for the network at hand \goto{back:asymNormality} \\
...
%     \setlength{\itemsep}{.75\baselineskip}
    ...
    $\to$ Could we 'learn' a deterministic transformation to, say, sample from it?
  \end{enumerate}
