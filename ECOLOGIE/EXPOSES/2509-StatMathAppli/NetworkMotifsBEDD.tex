...
\frame{\frametitle{Motif count}
  ...
  \begin{itemize}
    \item Determine the \emphase{$r_s$} {\sl automorphisms} = non-redundant permutations
    $$
    ...
    $$
    \item \pause Choose $p_s$ nodes among $m$ and $q_s$ nodes among $n$;
...
\frame{\frametitle{Motif probability} 
  ...
  \begin{align*}
    \overline{\phi}_s
    & = \emphase{\idotsint} \rho^4 g(u_1) g(u_2)^3 h(v_1) h(v_2) h(v_3)^2 \emphase{\d u_1 \d u_2 \d v_1 \d v_2 \d v_3} \\
...
\frame{\frametitle{Some more results}
  ...
  \item \pause \emphase{Variance:} Same game, requires to evaluate ${\Esp(N_s ^2) = \Esp\left(\sum_\alpha \text{match}_{s\alpha} \right)^2}$ \\
  ...
  \emphase{Proposition: Asymptotic normality \refer{OLR22}.}\footnote{Motif counts are also network $U$-statistics \refer{LeM23,LDM25}} Under {\BEDD}, for non-star motifs, 
  \begin{itemize}
%   \setlength{\itemsep}{.75\baselineskip}
  ...
  $$
  \item Account for plug-in when moderate network size ($\Delta$-method):
  $$
...
\frame{\frametitle{Model 1: Networks comparison in space and time} }
%====================================================================

%====================================================================
\frame{\frametitle{French plant-pollinator networks} 
...
%   \newcommand{\fignatB}{/home/robin/RECHERCHE/RESEAUX/GOF-Network/Motif_Analysis_Natasha/Article/Figures}
  ...
  \paragraph{Pollinator imbalance $D^{(g)}$.} Adonis anova table
  $$\small{
