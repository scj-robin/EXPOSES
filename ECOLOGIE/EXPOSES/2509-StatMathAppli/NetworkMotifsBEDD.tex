...
%       \end{array}$
  ...
  \begin{itemize}
    ...
    \item Expected degrees: $\Esp(Y_{i+} \mid U_i) = n \rho g(U_i)$, $\Esp(Y_{+j} \mid V_j) = m \rho h(V_j)$.
  \end{itemize} 
...
\frame{\frametitle{Motif count}
  ...
  \begin{itemize}
    ...
    \item \pause Try the $r_s$ {\sl automorphisms} = non-redundant permutations
    $$
...
\frame{\frametitle{Motif probability} 
  ...
  \bigskip \pause
  \paragraph{An example\footnote{Consider here induced motifs  (only the presence of the prescribed edges is required) $\neq$ exact motif}.} Consider the motif $s = \includegraphics[width=.06\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9}$ with $p_s = 2$ and $q_s = 3$, we have
  \begin{align*}
    \overline{\phi}_s
    & = \int\int\int\int\int \rho^4 g(u_1) h(v_3) g(u_2)^3 h(v_1) h(v_2) h(v_3) \d u_1 \d u_2 \d v_1 \d v_2 \d v_3 \\
    & = \left. \left(\int \rho^2 h(v_3) \d v_3\right) \left(\int \rho^3 g(u_2)^3 \d u_2\right) \right/ \rho \qquad \qquad \goto{back:motifProba}  \\
    & = \left. \left(\text{2-top star probability}\right) \times \left(\text{3-bottom star probability}\right) \right/ \left(\text{edge probability}\right)
  ...
  \begin{itemize}
    ...
    (which are the sufficient statistics of the BEDD model.)
...
\frame{\frametitle{Comparing networks in space and time} }
%====================================================================
