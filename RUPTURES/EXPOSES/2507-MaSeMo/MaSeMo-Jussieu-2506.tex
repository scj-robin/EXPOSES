...
\date[MaSeMo'25]{Markov, Semi-Markov Models and Associated Fields , Paris, Jul. 2025}
...
\frame{\frametitle{Decomposition of the count} 
  ...
  \begin{itemize}
    ...
    \item \emphase{$B_k =$ number of immigrants} within $I_k$: 
    $$
    B_k\sim \mathcal{P}(\mu)
    $$ 
    with $\mu = \lambda_0 \Delta$, 
    \item \emphase{$M_T(I_k) =$ number of descendants descendants of $T < \tau_k$} within $I_k$:
    $$
    M_T(I_k) 
    ...
    \item \emphase{$R_k =$ number of descendants of points $T \in I_k$} within $I_k$
  \end{itemize}
...
\frame{ \frametitle{Summary}
  ...
  \begin{itemize}
    ...
    ~ \\
    $\to$ The discretized Markov switching Hawkes process with exponential kernel is a hidden Markov model
...
\frame{ \frametitle{Non exponential kernel function $h$}
  ...
  \bigskip \bigskip 
  \paragraph{Markovian representation.} 
  Define $U_k = \sum_{\ell=1}^{(k-1)\wedge L} \alpha_\ell Y_{k-\ell}$,
  then
  $$
