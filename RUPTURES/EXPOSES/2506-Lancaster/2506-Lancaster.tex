...
%====================================================================
\frame{ \frametitle{Practical conclusions}

  \begin{itemize}
    \setlength{\itemsep}{1\baselineskip}
    \item Inference easier when more signal (large $\lambda$)!!!
    \item Inference easier with thinner discretization step (large $N$), but higher computational cost \\
    $\to$ recommendation: $N = 2 n$
    \item BIC does not capture the right number of states \\
    (but sequences not simulated according to the model)
    \item AIC does, with reasonable signal ($\lambda$) and discretization ($N$) \\
    (blind to the simulation shift from the model?)
