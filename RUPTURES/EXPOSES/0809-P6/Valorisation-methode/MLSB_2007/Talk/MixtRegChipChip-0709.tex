...
\noindent Study of the histone modification H3K9me3.

\noindent Very high density: more than 1,000,000 probes
($\approx$200,000 per gene)
...
ChIPOTle, and Mixture of regression (MMR).

\paragraph{False positive}
The chloroplastic genome is a negative control (no target).\\
$\bullet$ MMR: the BIC criterion select a single regression model,
i.e. no probe is declared enriched.\\
$\bullet$ ChIPOTle: finds X peaks.\\
$\bullet$ Nimblegen: not available.


\noindent$\bullet$ to test for true positives (Turck et al., 2007),

\noindent$\bullet$ to compare methods (Nimblegen, ChIPOTle).

\newpage

\paragraph{True positive}
Many identified targets biologically assessed (Turck et al., 2007).
\\
$\bullet$ MMR: At level $\alpha=0.01$, $\sim 20\%$ of the probes are
enriched on each chromosome.\\
$\bullet$ ChIPOTle: more than 90\% of redundancy with MMR \\
$\bullet$ Nimblegen: 


\section{NimbleGen tiling array of \textit{Arabidopsis thaliana}}
At level $\alpha=0.01$, $\sim 20\%$ of the probes are enriched on
each chromosome
