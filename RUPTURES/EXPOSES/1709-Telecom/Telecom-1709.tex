...
\date[Sept. 2017, Paris]{Télécom ParisTech, Sept. 2017, Paris}
...
\frame{\frametitle{Bayesian inference} 
  ...
  $$
  p(T\, | \,K) = \prod_{r \in T} a_r , \qquad \text{e.g. } a_r \propto n_r^\alpha
  $$
  \end{itemize}
...
%   (supposed to be easy to compute using e.g. conjugate priors) and
  ...
  $$
  \sum_{T \in \Tcal^K} \prod_{r \in T} a_r, \qquad \qquad \text{and} \quad \# \Tcal_{1:n}^K = {{n-1}\choose{K-1}}.
  $$
...
\frame{\frametitle{Gene regulatory network} 
  ...
  $$
  t \in r: \qquad Y_t \, | \, \theta_r   \sim \Ncal(\mu_r, \Sigma_r)
  $$
...
\frame{\frametitle{Hyper-Markov prior} 
  ...
  \onslide<4>{
    ...
    \ra multinomial-Dirichlet (conjugacy), normal-Wishart (conjugacy), Gaussian copulas (numerical integration), ...?
    }
  }
...
\frame{\frametitle{FMRI data \refer{CHA12} 2/2} 
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
      ...
      \begin{itemize}
      \item Some numerical issues (see later)
