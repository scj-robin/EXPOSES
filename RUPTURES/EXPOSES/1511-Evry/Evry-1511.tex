...
%     }
  ...
  \date[INGBI, Evry, 2015]{Statistical Analysis of Massive Genomic Data, Evry, Nov.'15}
...
\frame{\frametitle{Copy number variations}
  ...
  Copy number variations (CNV) are associated with (responsible for?) various
  types of cancers. \refer{Hup08}
  $$
  \begin{tabular}{cc}
...
%====================================================================
\frame{\frametitle{Different technologies}

\paragraph{CNV analysis with DNAseq:}
  \begin{itemize}
...
\frame{\frametitle{Inference problem}
  ...
  \item Segmentation (change-point locations): $T = (\tau_1, \dots,
    \tau_{K-1})$ \\
    \ra \textcolor{red}{discrete} parameters;
  \item Distribution parameters (within segment means): $\theta =
    (\theta_1, \dots, \theta_K)$ \\
    \ra \textcolor{blue}{continuous} parameters.
  \end{itemize}
...
\frame{\frametitle{Parameter inference} 
  ...
  \paragraph{When the change-pointsare known,} estimating the
  ...
  \pause\bigskip
  \paragraph{Finding the breakpoints:} We now have to find the change-points $\tau_1$, \dots, $\tau_{K-1}$ that maximize the log-likelihood
  $$
  ...
  \pause\bigskip
  \paragraph{Problem.} The number possible choices for the change-points grows exponentially:
  $$
...
\frame{\frametitle{Dynamic programming algorithm}
  ...
  \paragraph{Algorithm.} $S_K(i, j) =$ cost of the optimal segmentation of series $[i, j]$ into $K$ segments:
  \begin{eqnarray*}
...
\frame{\frametitle{Fastening the DP algorithm}
  ...
  \pause\bigskip
  \paragraph{Dynamic programming} uses the recursion
  $$
...
%   $$
  ...
  \bigskip \bigskip \pause
  \paragraph{Cumulated profile = } births and deaths process over $\{0, \dots, p\}$ with rates
  \begin{eqnarray*}
...
\frame{\frametitle{Distribution of the longest excursion of an $OU$ process}
  ...
  \begin{itemize}
   ...
   \item Re-weight each of them according to ${\dd P_{OU}}/{\dd P_B}$ given by the Girsanov formula.
  \end{itemize}
}
