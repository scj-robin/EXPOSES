...
\newcommand{\figsimul}{/home/robin/Bureau/Hawkes/HawkesDiscreteHMM/Figures/simuls}
...
\date[SAMM'25]{Séminaire du SAMM, Paris I, Jan. 2025}
...
\frame{\frametitle{Markovian reformulation} 
  ...
  \bigskip \bigskip 
  $(Y_k)_{k\geq 1}$ is \emphase{not a Markov chain} (infinite memory) but 
  \begin{itemize}
    \item \pause defining 
    \begin{align*}
      U_1 = 0, \qquad \qquad 
    ...
    \end{align*}
    \item \pause so \emphase{$\left((Y_k,U_k)\right)_{k \geq 1}$ forms a Markov Chain}.
  \end{itemize}
...
\frame{\frametitle{Discrete time Hawkes HMM} 
  ...
  \begin{itemize}
    ...
    \item The distribution of the number of offspring does not vary with the hidden path
  \end{itemize}
...
\frame{\frametitle{Discrete time Hawkes HMM} 
  ...
  \begin{itemize}
    ...
    \item The Markovian property still holds if the influence of the past varies with the hidden path ($\alpha \to \alpha_k, \beta \to \beta_k$).
  \end{itemize}
...
\frame{\frametitle{Inference}
  ...
  \end{align*}
  with $D_Q =$ number of parameters $= 2 + Q^2$ and $N =$ number of time bins.
}
...
\frame{ \frametitle{Simulation design}
  ...
  \begin{itemize}
    ...
    $$
    (H_t)_{0 \leq t \leq 1} \sim Heterogeneous~Continuous~Hawkes(a, b^0, m)
    $$
...
\frame{ \frametitle{Simulation results ($Q^* = 3$)}
  ...
  \begin{overprint}
    ...
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q3-Coef-discParms}
    \onslide<2> \paragraph{Model selection: BIC.} Distribution of $BIC_Q - BIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q3-Coef-BIC}
    \onslide<3> \paragraph{Model selection: AIC.} Distribution of $AIC_Q - AIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q3-Coef-AIC}
    \onslide<4> \paragraph{Comparison with Poisson HMM.} $AIC_{1, \widehat{Q}, Q^*}(Poisson)$ / $AIC_{1, \widehat{Q}, Q^*}(Hawkes)$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q3-Coef-compAIC}
  \end{overprint}
}
...
\frame{ \frametitle{Bat cries}
  ...
  \paragraph{Data set.} $\sim 1000$ overnight recordings.
  
  \bigskip 
  \begin{overprint}
    \onslide<2> \paragraph{Model selection.} $n = $ nb events, $N = c \; n$, \quad top: $BIC$, bottom: $AIC$. \\
    \includegraphics[width=.9\textwidth, height=.7\textheight]{\figchiro/ResSeqData-Coef-Qhat}
  \end{overprint}
}
...
\frame{ \frametitle{Summary}
  ...
  \begin{itemize}
    \setlength{\itemsep}{1.5\baselineskip}
    \item The discretized Markov switching Hawkes process with exponential kernel is a hidden Markov model
    \item The standard EM machinery applies to achieve maximum likelihood inference.
    \item Not shown: initialization based on existing tools for homogeneous Hawkes and Poisson HMM
  \end{itemize}
}
...
\frame{ \frametitle{Discussion}
  ...
  \begin{itemize}
    ...
    \item Understand the inferred latent states. \\
    (different animal behavior, species, ...)
  \end{itemize}
...
%====================================================================
\frame{ \frametitle{Simulation results ($Q^* = 1$)}

  \begin{overprint}
    \onslide<1> \paragraph{Parameter estimates.} Distribution of $\widehat{\theta}/\theta^*$ ($Q = Q^*$) \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q1-Coef-discParms}
    \onslide<2> \paragraph{Model selection: BIC.} Distribution of $BIC_Q - BIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q1-Coef-BIC}
    \onslide<3> \paragraph{Model selection: AIC.} Distribution of $AIC_Q - AIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q1-Coef-AIC}
    \onslide<4> \paragraph{Comparison with Poisson HMM.} $AIC_{1, \widehat{Q}, Q^*}(Poisson)$ / $AIC_{1, \widehat{Q}, Q^*}(Hawkes)$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q1-Coef-compAIC}
  \end{overprint}
}
    
%====================================================================
\frame{ \frametitle{Simulation results ($Q^* = 2$)}

  \begin{overprint}
    \onslide<1> \paragraph{Parameter estimates.} Distribution of $\widehat{\theta}/\theta^*$ ($Q = Q^*$) \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q2-Coef-discParms}
    \onslide<2> \paragraph{Model selection: BIC.} Distribution of $BIC_Q - BIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q2-Coef-BIC}
    \onslide<3> \paragraph{Model selection: AIC.} Distribution of $AIC_Q - AIC_1$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q2-Coef-AIC}
    \onslide<4> \paragraph{Comparison with Poisson HMM.} $AIC_{1, \widehat{Q}, Q^*}(Poisson)$ / $AIC_{1, \widehat{Q}, Q^*}(Hawkes)$ \\
    \includegraphics[width=.9\textwidth, height=.8\textheight]{\figsimul/SimHHMM1DV6-parmsSeed1-T1-Q2-Coef-compAIC}
  \end{overprint}
}
