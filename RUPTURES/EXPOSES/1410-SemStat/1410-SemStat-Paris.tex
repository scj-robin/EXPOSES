...
%====================================================================
\title[Identifiability in HMM]{Identifiability in Hidden Markov Models \\ 	Applications to Genomics}
...
\institute[AgroParisTech / INRA]{
 ...
 \begin{tabular}{ccccc}
  ...
  \date[SemStat]{S\'eminaire parisien de statistique, Paris, Octobre 2014}
...
\frame{\frametitle{State space models}
  ...
  \paragraph{Some exceptions are known,} e.g. non-parametric emission distributions:
   \begin{itemize}
   \item with shape constraints \refer{BMV06}, \refer{GaR13}, 
   \item in presence of known components \refer{BDV06}, \Refer{R. {\sl et al.} (2007)}\nocite{RBD07},
...
\frame{\frametitle{A previous result}
  ...
  \paragraph{\refer{AMR09}, Theorem 9:} The mixture with product emissions
  $$
  ...
  The proof relies on a result of \refer{Kru77} for 3-way contingency tables. See also \refer{Rho09}.
 }
...
%====================================================================
\subsection{Non-parametric HMM for counts}
\frame{\frametitle{Ex. 1: Non-parametric HMM for count data}
...
\frame{\frametitle{Simulated RNA-seq: Regularization}
  ...
  \end{tabular}
  $$
  Classification (rand-index) can be improved with regularization $\lambda$. 
  $$
  \text{\ra How to choose $\lambda$?}
  $$
  }
  
%====================================================================
\subsection{Non-parametric HMM with kernel density estimate}
\frame{\frametitle{Ex. 2: Non-parametric HMM with kernel density estimate}

\paragraph{Weighted kernel density estimate.} For some kernel $H_w$:
$$
\widehat{\phi}_k(\cdot) = \sum_t \rho_{tk} H_w\left({\cdot - Y_t}\right), 
\qquad
\sum_t \rho_{tk} = 1.
$$
The estimation of the $\phi_k$'s amounts at this of the $\rho_{tk}$'s.

\bigskip \pause
\paragraph{EM inference.} Denote $H = $ Gram matrix: $H_{tt'} = H(Y_t - Y_{t'})$.
\begin{enumerate}
\item E-step $\rightarrow \; \tau^h_{tk} =$ estimate of $\Pr\{S_t = k|Y\}$
\item M-step: quadratic complexity
$$
\gamma^h_{tuk} = {\rho^h_{uk} H_{tu}}\left/{\sum_v \rho^h_{vk} H_{tv}}\right., 
\qquad
\rho^h_{tk} = {\sum_u \tau^h_{uk} \gamma^h_{utk}}\left/{\sum_{u, v} \tau^h_{uk} \gamma^h_{uvk}}\right..
$$
\end{enumerate}

\pause
\paragraph{1 remark + 1 question.}
\begin{itemize}
 \item $\rho^h_{tk} \neq \tau^h_{tk}$ as proposed by \refer{JiM06}, \refer{C0C00}, \refer{BCH09} or, implicitly by \Refer{R. {\sl et al.} (2007)}\nocite{RBD07}.
 \item How to choose the window width $w$? Should we take $w_k$?
\end{itemize}

}

%====================================================================
\frame{\frametitle{Ex. 2: Seed/Leaf comparison}

  \paragraph{Tiling array experiment.}
  \begin{itemize}
   \item Tiling array: probe regularly spread along the genome ({\sl A. Thaliana}).
   \item 4 groups are expected but 5 are actually needed.
  \end{itemize}
  $$
  \begin{array}{ccc}
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-Groups} &
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-K5-k3-org-surf} &
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-K5-k4-org-surf} \\
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-K5-k2-org-surf} &
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-K5-k1-org-surf} &
   \includegraphics[width=.25\textwidth]{../Figures/data2_10dap_5000-K5-k5-org-surf}
  \end{array}
  $$
%   \begin{tabular}{cc}
%   \hspace{-0.05\textwidth}
...
%  \paragraph{Inference.}
%  \begin{itemize}
%   \item A generalized EM algorithm can be derived to estimate the $\rho_{tk}$ \refer{GCR14}.
...
%====================================================================
\frame{\frametitle{Genomic alterations in tumor cells}
...
% \newcommand{\figPopova}{/home/robin/RECHERCHE/RUPTURES/Exposes/Figures}
\renewcommand{\chr}{11}
  ...
  \begin{tabular}{cc}
    ...
    \onslide+<1->{\paragraph{SNP calling:} Classify into one genotype = genotyping:
      $$
	...
	\includegraphics[width=.5\textwidth, clip=]{../Figures/R_BLC_B1_T45-Chr\chr-LRR} \\
	Data from \refer{PMS09}, chromosome \chr.
      \end{tabular}
      \onslide<3>
      \begin{tabular}{c}
	$BAF_t = f(\text{locus } t)$ \\
	\includegraphics[width=.5\textwidth, clip=]{../Figures/R_BLC_B1_T45-Chr\chr-BAF} \\
	Data from \refer{PMS09}, chromosome \chr. \\~ \\~
      \end{tabular}
    \end{overprint}
  \end{tabular}
...
%====================================================================
% \newcommand{\figdir}{/home/robin/RECHERCHE/RUPTURES/Exposes/Figures}
...
%     \hspace{-1cm}
%     \begin{tabular}{p{.5\textwidth}}
%       \includegraphics[width=.5\textwidth, height=.45\textheight]{../Figures/\figname-LRR}
%     \end{tabular} \\ \pause
%     \hspace{-0.5cm}
%     \begin{tabular}{p{.5\textwidth}}
%       \includegraphics[width=.5\textwidth, height=.45\textheight]{../Figures/\figname-BAF}
%     \end{tabular}
%     &
%     \hspace{-1cm}
%     \begin{tabular}{p{.5\textwidth}}
...
%       $$BAF_t = B_t / (A_t + B_t)$$
%       reveals loss of heterozygocity.
%     \end{tabular} \\    
%   \end{tabular}
% }
% 
...
% \renewcommand{\figres}{R_BLC_B1_T45-Chr11-c3-5-Gam}
\renewcommand{\figres}{\figname}
...
\frame{\frametitle{Data: 3 representations}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
      \vspace{-.35cm}
      \onslide+<4->{
      \includegraphics[width=.5\textwidth, height=.45\textheight]{../Figures/\figname-AB}
      }
    \end{tabular} \\    
  \end{tabular}
}
...
\frame{\frametitle{CNV/LOH status}
  ...
  \paragraph{Genotype.} Could be coded at each locus as $(a, b)$:
  $$
  \text{Ex:} \qquad
  \text{AA} = (1, 1), \quad
  \text{AAB} = (2, 1), \quad
  \text{ABBB} = (1, 3),
  $$
  but we are actually note interested in recovering it.
...
%====================================================================
\newcommand{\figdir}{/home/robin/RECHERCHE/RUPTURES/LOH/Fig}
...
\frame{\frametitle{Genotype}
  ...
  \item \pause Denoting $p$ the frequency of allele A at the current locus ($p = p_t$):
  {\footnotesize{
  $$
  ...
  (1, 0) & & p & q \\
  (2, 0) & & & & p &  & q\\
  \emphase{(2, 1)} & & & & \emphase{p^2} & \emphase{2pq} & \emphase{q^2}\\
  (3, 0) & & & & & & & p & & & q \\
  (3, 1) & & & & & & & p^2 & pq & pq & q^2 \\
  (4, 0) & & & & & & & & & & & p & & & & q \\
  (4, 1) & & & & & & & & & & & p^2 & pq & & pq & q^2 \\
...
\frame{\frametitle{HMM with mixture emissions}
  ...
  \bigskip \pause
  \paragraph{HMM approach:} because of the classification purpose, as opposed to \refer{PMR13} who use a non-parametric segmentation approach.
...
%   \paragraph{Previous Theorem \refer{GCR14}.} Hidden Markov models are identifiable as soon as the emission distributions are linearly independent.
%   
%   \bigskip
%   {\sl Sketch of proof.} Consider the joint distribution of $(Y_{t-1}, Y_t, Y_{t+1})$. The rest follows from \refer{AMR09}

%  \pause \bigskip \bigskip
  \paragraph{Corollary.} The proposed HMM is identifiable.
%   as soon as the emission distributions $f_z$ are linearly independent.
  
  \bigskip \bigskip \bigskip 
  \paragraph{Proof.} The identifiability theorem applies because
  \begin{itemize}
  \item the rows of matrix $\nu = [\nu_{sz}]$ are linearly independent and
...
\frame{\frametitle{Inference}
  ...
  \pause \bigskip %\bigskip
  \paragraph{E step.}   $(S_t, Z_t)$ is a Markov chain with transition matrix 
  $$
  P_{(s,z); (s',z')} = \pi_{s; s'} \nu_{s', z'}
  $$
  \ra The forward-backward recursion directly applies.
...
\frame{\frametitle{Model selection}
  ...
  The limits $c_{\min}$ and $c_{\max}$ have to be chosen. 
  ...
  = BIC - \mathbb H(H | Y) 
  $$
  where $H$ stands for the hidden variables \refer{BCG00}.
  
  \pause \bigskip \bigskip
  \paragraph{Dedicated version.} Here $H = (S, Z)$ but, as we are only interested in $S$, we use
  \begin{eqnarray*}
  ICL = BIC - \mathbb H(S | Y) 
  \end{eqnarray*}
  which does not penalize for the posterior entropy of $Z$ \refer{VBM13}.
  }
...
\renewcommand{\figdir}{/home/robin/RECHERCHE/RUPTURES/LOH/ResHMMLOH2/Popova}
...
% %====================================================================
% \renewcommand{\figdir}{/home/robin/Bureau/LOH/Data/Popova}
% 
% \renewcommand{\comment}{Bla bla bla}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c3-5-Gam} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{Gaussian / BIC}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c3-4} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{Gaussian / ICL}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c2-3} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{Gamma / BIC}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c3-4-Gam} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{Gamma / ICL}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c4-4-Gam} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{symmetric Gamma / BIC}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c3-4-GamSym} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{symmetric Gamma / ICL}
% \renewcommand{\figname}{R_BLC_B1_T45-Chr11-c4-4-GamSym} 
% \frame{\frametitle{Example 1}\input{PlotExample.tex}}
...
%   \paragraph{Model choice matters:} 
%   $$
%   \max ICL: c_{\min} = 0, c_{\max} = 3
%   $$
%   Chromosome 6 from \refer{SLV08}
% }
% \renewcommand{\figname}{NA06991-BAF-R-Chr6-c0-3}
% \frame{\frametitle{Model choice}\input{PlotExample.tex}}
% 
% \renewcommand{\comment}{      
%       \paragraph{Model choice matters:} 
%       $$
%       (c_{\min}, c_{\max}) = (2, 2) \text{ looks better}
%       $$
%       Chromosome 6 from \refer{SLV08}
% }
% \renewcommand{\figname}{NA06991-BAF-R-Chr6-c2-2}
% \frame{\frametitle{Model choice}\input{PlotExample.tex}}
...
% 
% \frame{\frametitle{In practice (1/3)}
% 
%   \begin{tabular}{cc}
%     \hspace{-0.5cm}
%     \begin{tabular}{p{.45\textwidth}}
...
% %====================================================================
% \frame{\frametitle{In practice (2/3)}
% 
%   \begin{tabular}{cc}
%     \hspace{-0.5cm}
%     \begin{tabular}{p{.45\textwidth}}
...
% %====================================================================
% \frame{\frametitle{In practice (3/3)}
% 
%   \begin{tabular}{cc}
%     \hspace{-0.5cm}
%     \begin{tabular}{p{.45\textwidth}}
...
% \frame{\frametitle{Joint CNV/LOH analysis via HMM-mixture}
% 
%   \paragraph{Summary:}
%   \begin{itemize}
...
%    \item Regularized transition matrix?
...
\frame{\frametitle{Conclusion}
  ...
  \paragraph{Identifiability of HMMs} allows more flexible emission distributions than for mixtures:
  \begin{itemize}
   \item Non parametric,
   \item Mixtures,  ...
  \end{itemize}
  ...
  \bigskip \bigskip \pause
  \paragraph{CNV/LOH classification:}
  \begin{itemize}
   \item A (first?: \refer{CXZ11}) joint modeling for CNV and LOH analysis.
   \item A standard framework for parameter inference and classification.
  \end{itemize}

  \bigskip \bigskip \pause
  \paragraph{A series of theoretical \& practical issues:} 
  \begin{itemize}
   \item Model selection (how many states? components?),
   \item Regularization (for non-parametric distributions),
   \item Practical behavior of EM (increasing number of local maxima?) ... \\
   \ra Are we stuck with EM?
  \end{itemize}
