...
%====================================================================
\title[Segmentation and CNV]{Segmentation methods for the detection of copy number variations}
...
\institute[AgroParisTech / INRA]{UMR 518 AgroParisTech / INRA Applied
  ...
  \date[APT Doctoral Course]{AgroParisTech Doctoral Course, July 2012}
...
% ====================================================================
\frame{ \frametitle{Copy number variations} 
  
  \paragraph{'Copy number variations' (CNV)} refer to 
  \begin{itemize}
  \item departure from diploidy (or tetra-),
  \item at a given locus,
  \item with respect to a reference genome.
  \end{itemize}
  
  \bigskip\pause
  \paragraph{Two leading examples:}
  \begin{itemize}
  \item \emphase{Human:} Genomic alterations are associated with
    various types of cancers.
  \item \emphase{Plants:} Genome organization can be associated with
    different phenotypes accross lines.
  \end{itemize}
}
...
\frame{\frametitle{Cancer cell genomic alterations}
  ...
  \bigskip
  CGH technology give access to CNV, but not allow the positioning of
  translocations.  (\refer{Hup08})

}

%====================================================================
\frame{\frametitle{CNV in Maize}
  \begin{tabular}{cc}
    \hspace{-.5cm}
    \begin{tabular}{p{.35\textwidth}}
      \paragraph{Genomic comparisons} 
      of {\sl A. thaliana} ecotypes or maize lines strain 
      \begin{itemize}
      \item Access \emphase{structural
        variations}
      \item Relation with phenotypes.
        \end{itemize}
    \end{tabular} 
    &
    \hspace{-.8cm}
    \begin{tabular}{c}
      \epsfig{file = ../Figures/SYF09-PLoSGenetics-Fig4.ps, clip=,
        width=.8\textwidth} \\
      \refer{SIF09}
    \end{tabular}  
  \end{tabular} 
  ...
  \medskip \pause
  \paragraph{Which variations?} 
  Most experiments are carried using one \emphase{'reference' genome}
  (e.g. wild-type, one given line) \\
  \ra Only variations with respect to the reference can be observed.
...
%    \begin{tabular}{c}
%      \epsfig{file = ../Figures/SYF09-PLoSGenetics-Fig4.ps, clip=,
%        width=.8\textwidth, bbllx=375, bblly=340, bburx=580,
%        bbury=390}
%    \end{tabular} \\
%    \refer{SIF09}
%  \end{tabular} 
...
%  \begin{tabular}{cc}
%    \begin{tabular}{c}
%      \epsfig{file = ../Figures/SEK10-GeomeRes-Fig1A.ps, clip=,
%        height=.8\textheight}
%    \end{tabular} \\
%    \refer{SEK10}
%  \end{tabular} 
...
\frame{\frametitle{Segmentation model for one single profile}}
%====================================================================
...
%====================================================================
\frame{\frametitle{Segmentation model (one profile)} \pause
...
\frame{\frametitle{Maximum likelihood} 
  ...
  $$
  P(\Ybf; T; \thetabf) = \prod_k \prod_{t \in I_k} p(Y_t, \theta_k)
  $$

  \bigskip\pause
  \paragraph{Array CGH.} Gaussian with same variance $\Ncal(\mu_k, \sigma^2)$:
  $$
  \log P(\Ybf; T; \thetabf) = \text{cst} - \frac{n}2 \log \sigma^2 -
  ...
  $$
  \log P(\Ybf; T; \thetabf) = \text{cst} - \sum_k \sum_{t \in I_k}
  (\mu_k - Y_t \log \mu_k)
  $$
  }
  ...
  J_K(1, n) = \left\{
    \begin{array}{ll}
      \displaystyle{\sum_{k=1}^K \sum_{t \in I_k} (Y_t -
        \widehat{\mu}_k)^2} & \quad \text{for } \Ncal(\mu_k, \sigma^2)\\
      \displaystyle{\sum_{k=1}^K \sum_{t \in I_k} (\widehat{\mu}_k -
        Y_t  \log\widehat{\mu}_k)}  & \quad \text{for } \Pcal(\mu_k)
    \end{array}
  \right.
  $$
...
\frame{\frametitle{Fastening the DP algorithm}
  ...
  \pause
  \paragraph{Trick.} Not to compute parameter estimates (see \#
...
\frame{\frametitle{Model selection: $K = ?$} 
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.45\textwidth}}
	  ...
	  \bigskip 
	  \paragraph{Penalized constrast $=$} most common strategy:
	  $$
	  \onslide+<4->{\textcolor{blue}{\arg\min_K}}
	  ~\onslide+<2->{J_K(1, n)}
...
%====================================================================
\frame{\frametitle{Some criteria} 
  
  \paragraph{AIC or BIC} 
  do not apply because segmentation is a non standard case, e.g.
  $$ 
  %
  \log p(K | Y) = \text{cst} + \underset{\approx \text{BIC}}{\underbrace{\log
      \int p(Y |K, \theta) p(\theta|K) \dd \theta}}
  $$
  but the Laplace approximation does not hold for the segmentation model.
  %        the number of possible segmentation increases as $\Ocal(n^K)$.
  
  \bigskip \pause
  \paragraph{Numerous specific criteria} have been derived:
  \begin{itemize}
  \item penalized likelihood \refer{Leb05}:
    $\displaystyle{
    \widehat{J}_K(1, n) + \text{pen}\left(K, \binom{n-1}{K-1}\right)
    }
    $
  \item adaptive penalty \refer{Lav05};
  \item modified BIC \refer{ZhS07};
  \item exact ICL \refer{RLR11};
  \end{itemize}
  
  \pause
  This turns out to be the \emphase{most delicate problem} in practice...  
  (see \# \ref{Page:Lasso}).
}
...
%%   \item Or use HMM (see \# \ref{Page:HMM}).
%%   \end{itemize}
%%   }
...
%====================================================================
\frame{\frametitle{Bayesian perspective} 

  \paragraph{Bayesian inference.} Given
  \begin{itemize}
  \item a prior distribution on the segmentation $\Tbf$: $P(\Tbf)$,
  \item a prior distribution on the parameters $\thetabf$: $P(\thetabf)$,
  \item a generative model for the observed data $\Ybf$: : $P(\Ybf | \Tbf, \thetabf)$,
  \end{itemize}
  Bayesian inference relies on the posterior (or conditional) distribution
  $$
  P(\Tbf, \thetabf | \Ybf)
  \quad \rightarrow \quad
  P(\tau_1 |\Ybf)
  \quad \rightarrow \quad
  \text{credibility interval for $\tau_1$}.
  $$
  
  \bigskip \pause
  \paragraph{Specific case of segmentation.} 
  Because the 'segmentation space' is huge, we would like to avoid
  time-consuming MCMC strategies to sample in the posterior
  distribution:
  $$
  \{(\Tbf^1, \thetabf^1), \dots (\Tbf^b, \thetabf^b), \dots (\Tbf^B,
  \thetabf^B)\} \sim P(\Tbf, \thetabf | \Ybf)
  $$
  }

%====================================================================
\frame{\frametitle{Combinatorial remark} 

  \paragraph{Combinatorial issue.} 
  The calculation of $p(\Tbf | \Ybf)$ requires these of 
  \begin{eqnarray*}
  P(\Ybf) & = & \sum_\Tbf P(\Ybf | \Tbf) P(\Tbf) \\ & = &
  \sum_{\tau_1, \tau_2, \tau_{K-1}} P(\Ybf_1^{\tau_1})
  P(\Ybf_{\tau_1+1}^{\tau_2}) \dots P(\Ybf_{\tau_{K-1}+1}^n) P(\tau_1,
  \tau_2, \tau_{K-1})
  \end{eqnarray*}
  which has a \emphase{sum-product} form.

  \bigskip \pause
  \paragraph{Exact calculation of $P(\Tbf | \Ybf)$.} 
  Sum-product forms typically occur in matrix products:
  $$
  \left[\Abf \times \Bbf\right]_{i, k} = \sum_j a_{ij} b_{jk}
  $$ 
  \ra the calculation of $P(\Tbf | \Ybf)$ amounts to compute $\Abf^K$,
  where $\Abf$ is a $(n+1) \times (n+1)$ matrix: \emphase{complexity =
    $\Ocal(Kn^2)$} (\refer{RLR11}).  
}
...
\frame{\frametitle{Lasso-like techniques}}
%====================================================================
...
%====================================================================
\frame{\frametitle{Application to CNV}
...
\frame{\frametitle{What about calling?}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
      Pure segmentation \\
      \epsfig{file = ../Figures/FigSegClas-1.eps, clip=,
        width=0.4\textwidth, height=0.25\textwidth}
    \end{tabular} 
    &
    \hspace{-1cm}
    \pause
  ...
  \bigskip\pause
  \paragraph{Exercise:} Write a classification-segmentation model (hint: see
  \refer{PRL07}).
  
  \bigskip \pause
  \paragraph{Alternative modeling:} Hidden Markov Models (HMM).
  }

%====================================================================
\frame{\frametitle{HMM: Another model for the same problem}
  ...
  \begin{tabular}{cc}
    ...
    \end{tabular}
  \end{tabular} \\
  \onslide+<6->{(Although we know we are unlikely to succeed.)}
...
\frame{\frametitle{Parameter inference}
  ...
  \paragraph{Aim.} We want to infer
  \begin{itemize}
  \item the parameters $\thetabf = (\{\pi_{q\ell}\}, \{\mu_q\}, \{\sigma_q\})$ and
  \item the labels $\Zbf = \{Z_1, Z_2, \dots Z_t, \dots\}$
  \end{itemize}

  \bigskip \pause
  \paragraph{Incomplete data model.} Parameter inference would be easy
  if the labels $\Zbf$ where known ... but they are not. 
  \bigskip
  Maximum likelihood inference requires to deal with the
  \emphase{incomplete likelihood}
  $$
  \log P(\Ybf; \thetabf) = \log \sum_\zbf P(\Ybf,  \zbf ; \thetabf)
  $$ 
  where the sum is taken over \emphase{all possible labelings $\zbf$},
  that is $Q^n$ possibilities.

  \bigskip \pause
  \paragraph{EM trick:} try to rather deal with the \emphase{complete likelihood}: 
  $$
  \log P(\Ybf; \thetabf) = \Esp\left[\log P(\Ybf,  \Zbf ; \thetabf) | \Ybf \right] + \Hcal(P(\Zbf | \Ybf)) 
  $$
}
...
\frame{\frametitle{Parameter inference}
  ...
  \item \emphase{E-step:} Compute the probability for each probe $t$
    to have label $q$:
    $$ 
    P(\Zbf | \Ybf) \qquad \longrightarrow \qquad \tau_{tq} =
    \Pr\{Z_t = q \;|\; \Ybf\},
    $$
    which this can be done via the \emphase{Forward-Backward} algorithm;
...
\frame{\frametitle{Graphical model}
  ...
  The main issue is to compute the conditional distribution of the
  labels $\Zbf$ given the observation $\Ybf$.

  \bigskip
  \onslide+<2->{The model states that}
  \begin{itemize}
  \item \onslide+<3->{the label sequence $(Z_t)$ is a Markov chain;}
  \item \onslide+<4->{the observations $(X_t)$ are independent given the labels;}
  \end{itemize}
  \onslide+<5->{
    \paragraph{Consequence:} 
    the label sequence $(Z_t)$ is \emphase{still a Markov chain},
    conditionally to the observations.}
  \begin{overprint}
    \onslide<3>
    $$
    \epsfig{file = ../Figures/HMM1-Z.eps, width=.8\textwidth, clip=}
    $$
    \onslide<4>
    $$
    \epsfig{file = ../Figures/HMM1-ZX.eps, width=.8\textwidth, clip=}
    $$
    \onslide<5>
    $$
    \epsfig{file = ../Figures/HMM1-ZcondX.eps, width=.8\textwidth, clip=}
    $$
  \end{overprint}
  }
...
%   here:
%   \begin{itemize}
%   \item Efficient heuristics to speed up the algorithms;
...
%====================================================================
%====================================================================
\section{Multiple profiles}
\frame{\frametitle{Multiple profiles}}

%====================================================================
\frame{\frametitle{Multiple comparisons}

  CNV are often studied accross a (large) set of individuals:
  \begin{itemize}
  \item \emphase{Human:} Find \emphase{recurrent genomic alterations} among
    patients having the same type of cancer.
  \item \emphase{Plants:} Compare lines or ecotypes to identify
    \emphase{structural variations} that can be associated with phenotypes.
  \end{itemize}

  \bigskip \pause 
  Most methods can be generalized to multiple profiles, but have to
  face \emphase{two main issues.}
  \begin{itemize}
  \item Breakpoints are likely to have the \emphase{different
    locations} accross all profiles.
  \item Observed profiles are likely to be \emphase{correlated}.
  \end{itemize}
  
}

%====================================================================
\frame{\frametitle{Dependence between the profiles}

  %\vspace{-0.5cm}
  \begin{tabular}{cc}
    \hspace{-0.5cm}
    \begin{tabular}{p{.45\textwidth}}
      The dependence between the profiles can be due
      \begin{itemize}
      \item \onslide+<2->{to the experimental design \\
        (e.g. comparative technologies)}
      \item \onslide+<4->{to the genetic similarity between the individuals \\
        (e.g. known or unknown pedigree)}
      \end{itemize} 
      ~\\ ~\\ ~\\ ~\\
    \end{tabular}
    &
    ...
    \begin{tabular}{c}
      \begin{overprint}
        \onslide<2> 
        \epsfig{file = ../Figures/LoopDesign.eps, width=.8\textwidth, clip=}
        \onslide<3> 
        \epsfig{file = ../Figures/StarDesign.eps, width=.8\textwidth, clip=}
        \onslide<4> 
        \epsfig{file = ../Figures/GenetDist.eps, width=.8\textwidth, clip=}
        %\onslide<5> 
        %\epsfig{file = ../Figures/Pedigree.eps, width=.8\textwidth, clip=}
      \end{overprint}
    \end{tabular}
  \end{tabular}
}
...
%====================================================================
\frame{\frametitle{Accounting for the dependence}
  
  Inference of both segmentation and HMM take advantage of a simple
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.4\textwidth}}
      \onslide+<2->{\paragraph{Segmentation.}}
        \begin{itemize}
        \item \onslide+<2->{Easy for independent profiles.}
        \item \onslide+<4->{Dynamic programming does not directly
          apply to dependent profiles.}
        \end{itemize}

      \bigskip
      \onslide+<5->{\paragraph{HMM.}}
        \begin{itemize}
        \item \onslide+<5->{Easy for independent profiles.}
        \item \onslide+<8->{Complex for numerous dependent profiles.}
        \end{itemize}
    \end{tabular}
    &
    \hspace{-1cm}
    \begin{tabular}{p{.5\textwidth}}
      \begin{overprint}
        ...
        \epsfig{file=../Figures/SegFA-ModGraph-Xcov.eps, clip=,
          width=0.7\textwidth}
        \onslide<5>
        \epsfig{file=../Figures/HMMmI-Z.eps, clip=,
          width=0.7\textwidth}
        \onslide<6>
        \epsfig{file=../Figures/HMMmI-ZX.eps, clip=,
          width=0.7\textwidth}
        \onslide<7>
        \epsfig{file=../Figures/HMMmI-ZcondX.eps, clip=,
          width=0.7\textwidth}
        \onslide<8>
        \epsfig{file=../Figures/HMMm-Z.eps, clip=,
          width=0.7\textwidth}
        \onslide<9>
        \epsfig{file=../Figures/HMMm-ZX.eps, clip=,
          width=0.7\textwidth}
        \onslide<10>
        \epsfig{file=../Figures/HMMm-ZcondX.eps, clip=,
          width=0.7\textwidth}
        \end{overprint}
...
%====================================================================
\frame{\frametitle{Segmentation model for multiple profiles}
...
\frame{\frametitle{Conditional distribution}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
        ...
        adds one layer of unobserved quantities (parameters).

        We are interested in
        $$
        P(\Zbf, \thetabf | \Ybf)
        $$
        which can not be computed efficiently.

        \onslide+<8>{
          \bigskip
          We are only able to
          \begin{itemize}
          \item Either \emphase{sample} in $P(\Zbf, \thetabf | \Ybf)$ with
            potentially time-consuming MCMC algorithms
          \item Or to \emphase{approximate} it (see e.g. variational
            Bayes, VB-EM, algorithms, \refer{BeG03}.)
          \end{itemize}
        }
    \end{tabular}
    & 
...
%\frame{\frametitle{Multiple profiles}
%
%  \paragraph{Aim.} Consider the profiles of $m$ patients (see \#
