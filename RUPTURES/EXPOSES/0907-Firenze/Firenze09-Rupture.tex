...
\begin{center}
  \textblue{\LARGE Statistical Analysis of CGH Arrays}
   ...
   \vspace{1.5cm}
   \paragraph{Joint work with} E. Budinska, E. Lebarbier, F. Picard,
   F. Radvanyi, C. Rouveirol, V. Stefanov, B. Thiam
    ...
    \begin{tabular}{ccccc}
      ...
      \hspace{1cm} &
      \epsfig{file=../Figures/logagroptechsolo.eps, width=3.7cm} &
      \hspace{1cm} &
...
Genomic alteration are asociated with (responsible for?) several
severe diseases.
$$
\begin{tabular}{cc}
  Normal cell & Cancer cell \\
  \epsfig{file = ../Figures/KaryotypeCancer-PH.ps, clip=,
    bbllx=325, bblly=676, bburx=468, bbury=780, scale=2}
  &
  \epsfig{file = ../Figures/KaryotypeCancer-PH.ps, clip=,
    bbllx=127, bblly=676, bburx=319, bbury=780, scale=2}
\end{tabular}
$$
Source: \refer{Hupé}{08}
...
\vspace{-1.3cm}
$$
\epsfig{figure=../Figures/MicroarrayTech.ps, height=15.5cm, width=19.5cm, clip=}
$$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\chapter{A model-based approach}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bigskip
\section{A model what for?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{itemize}
\item \vspace{-0.5cm} To \emphase{translate biological questions into
    mathematical equations} and quantities;
\item \vspace{-0.5cm} To make \emphase{all hypotheses explicit};
\item \vspace{-0.5cm} To set the inference of interesting parameters
  in a \emphase{global framework}, possibly accounting for other
  effects (covariates);
\item \vspace{-0.5cm} To \emphase{justify all calculations} and data
  processing to come.
\end{itemize}

\bigskip
\centerline{\framebox{The model is the place where biologist's and
  mathematician's minds meet.}}

\bigskip\bigskip
\paragraph{Our approach.}
\begin{itemize}
\item \vspace{-0.5cm} Define a model that describes the biological
  process as well as possible;
\item \vspace{-0.5cm} Make sure that it can be handled in terms of
  mathematics / statistics;
\item \vspace{-0.5cm} Derive an (efficient and statistically valid)
inference procedure.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{A mathematical representation of a CGH profile}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item The value of the true log-ratio $\lambda_t$ is affected by
  ...
  $$
  Position \emphase{$t_1$, $t_2$, ..} are called {\sl
    breakpoints}. \emphase{$\mu_k$} is the true log-ratio in segment
  \emphase{$I_k$}.
...
\item There are $\coefbin{n-1}{K-1}$ possible choices for the
  ...
  (e.g. \emphase{$n=1\,000, K = 20 \longrightarrow 6.8\;10^{39}$}): \\
  \\
...
\bigskip
\paragraph{Segmentation is not sufficient.}
$$
\begin{tabular}{cc}
...
\item We introduce \emphase{hidden variables $Z_{kp}$}, indicators of the
...
\item The \emphase{distribution of the signal} given the group of the
  ...
  \Ncal(m_{\emphase{p}}, s_{\emphase{p}}^2).
...
\begin{itemize}
\item \vspace{-0.5cm} The inference of the \emphase{breakpoints $T$}
  is made at the \emphase{position level $t$};
\item \vspace{-0.5cm} The inference of the \emphase{groups (status)
...
\begin{enumerate}
\item \vspace{-0.5cm} When $T$ is fixed, the
  \textblue{Expectation-Maximisation (EM)} algorithm estimates
  $\Theta$;
%   $$
...
%   \hat{T}^{(h)})
%   $$
\item \vspace{-0.5cm} When $\Theta$ is fixed, \textblue{dynamic
...
$$
\log \Lcal_{KP}(\hat{\Theta}^{(\emphase{h})}; \hat{T}^{(\emphase{h})}) 
\leq \log \Lcal_{KP}(\hat{\Theta}^{(\emphase{h+1})};
\hat{T}^{(\emphase{h})}) 
\leq \log \Lcal_{KP}(\hat{\Theta}^{(\emphase{h+1})};
\hat{T}^{(\emphase{h+1})}) 
$$
...
$$
  \begin{tabular}{cc}
    ...
    \textblue{\pi_3 f_3(x)}$ & $\tau_{kp} = \Pr\{k \in p \;|\;
    Y_k\}
    = \pi_k f_p(y_k) / f(y_k)$\\
    \\
...
$$  
\begin{array}{cccc}
  \quad \tau_{kp}~~(\%) \quad & \qquad g=1 \qquad & \qquad g=2 \qquad &
  \qquad g=3 \qquad \\
  \hline
  k = 1 & 65.8 & 0.7 & 0.0 \\
  k = 2 & 34.2 & 47.8 & 0.0 \\
...
observed on individual from different genotypes. ({\sl Supported by
  ANR/Génoplante AgriArray project}.)
...
\begin{itemize}
\item \vspace{-0.5cm} Segment each patient $i$ with any number of
  breakpoints $K_m$;
\item \vspace{-0.5cm} Find the \emphase{optimal repartition} of $K$ between
  the $m$ patients.
\end{itemize}
...
\item \vspace{-0.5cm} Maximum likelihood for the \emphase{complete
  ...
  \centerline{$\rightarrow$ Approximations (\emphase{variationnal
...
\vspace{-0.5cm} 
\begin{itemize}
\item High density: tiling arrays ($\simeq 10^5$
  probes per chromosomes) \\
  \centerline{$\longrightarrow$ Define heuristics (CART, AHC) to reduce
    the computational burden.}
\item \vspace{-0.5cm} High throughput sequencing (discrete signal)  \\
  \centerline{$\longrightarrow$ Need for new statistical models?}
\end{itemize}
