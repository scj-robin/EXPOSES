...
\institute[AgroParisTech / INRA]{UMR 518 AgroParisTech / INRA Applied
  ...
  \date[SMAI-MAIRCI]{Journées SMAI-MAIRCI \\ Grenoble, September 2012}
...
%====================================================================
\subsection{Several Series}
%====================================================================
\frame{\frametitle{Several Series}
...
\frame{\frametitle{Algorithmic Issue}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.4\textwidth}}
      %\vspace{-1cm}      
      \onslide+<1->{\paragraph{One series:} 
      ...
      \onslide+<3->{\medskip
        ...
        same complexity as for one series \ra classical DP. \\}
      \onslide+<4->{\medskip
...
\frame{\frametitle{Posterior Distributions}
  ...
  The calculation of all these quantities require this of
  $$
...
\frame{\frametitle{Calculating Exact Posterior Distributions}
  ...
  & = & \sum_{T \in \Mcal_K} \prod_{r \in T} P(Y^r)
  \end{eqnarray*}

%  \pause
%  \paragraph{Localisation of the $k$-th breakpoint:}
%  \begin{eqnarray*}
%    \Pr\{\tau_k = t | K, \Ybf\} & \propto & \left(\sum_{T \in \Mcal_k(1, t)} \prod_{r
%    \in T} P(Y^r)\right) \left(\sum_{T \in \Mcal_{K-k}(t+1, n)} \prod_{r
%    \in T} P(Y^r)\right)
%  \end{eqnarray*}
...
\frame{ \frametitle{Exploring the segmentation space}
  ...
  $\sum_{T \in \Mcal_K} \prod_{r \in T} P(Y^r)$ can be computed as
  $$
...
\frame{ \frametitle{Model selection: Exact BIC and ICL}
  ...
  $$
  \log P(\Ybf, K) 
%  = \log \left[\emphase{\sum_{T \in \Mcal_K}} P(T) \int P(\Ybf | T,
