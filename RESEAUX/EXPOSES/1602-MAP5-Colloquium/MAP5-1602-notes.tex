...
\end{eqnarray*}
\begin{itemize}
 \item Etape VE: calcul de la loi approchée 
 $$
...
\paragraph{Application au SBM.} On note $Z_{ik} = \mathbb{I}\{Z_i = k\}$. Borne inférieure de la log-vraisemblance~:
$$
...
%----------------------------------------------------------------------
\subsection{Ajustement à un modèle}
%----------------------------------------------------------------------
...
\begin{itemize}
 \item Problème d'identifiabilité de $\gamma(\cdot, \cdot)$~: difficile d'interpréter
 \item Cas limite $\gamma(\cdot, \cdot) \equiv p$ correspond à $ER(p)$.
\end{itemize}

\paragraph{Modèle de régression logistique.}
$$
(Y_{ij})_{i, j} \text{ indep.}: \qquad 
Y_{ij} \sim \Bcal[\logit(x_{ij} \beta)]
$$
Introduction d'un terme résiduel de sous forme de graphon
$$
(U_i)_i \text{ i.i.d.}, \quad 
(Y_{ij})_{i, j} \text{ indep.}: \qquad 
Y_{ij} \sim \Bcal[\logit(x_{ij} \beta) + \gamma(U_i, U_j)]
$$
Introduction d'un terme résiduel de sous forme d'une combinaison de SBM à $K = 1, 2, \dots$ classes~:
$$
(U_i)_i \text{ i.i.d.}, \quad 
(Y_{ij})_{i, j} \text{ indep.}: \qquad 
...
\paragraph{Test d'ajustement.} Inférence par VBEM
$$
\pt(K) \simeq p(K|Y)
$$
'Goodeness-of-fit' mesuré par $\Prt\{K = 1\} \simeq \Pr\{K=1|Y\}$.
