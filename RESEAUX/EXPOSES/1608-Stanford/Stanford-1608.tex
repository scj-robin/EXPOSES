...
\frame{\frametitle{Variational Bayes inference: Some tools}
  ...
  $$
  \min_{\pt} \; KL[\pt(\cdot, \cdot) \ \| \ p(\cdot, \cdot|Y)] 
...
%   \ra Computable lower bound of $p(Y)$ 
  ...
  \quad \propto \quad p(K) ~ e^{\log p(Y|K) - KL_K^*} 
  $$ 
...
\frame{ \frametitle{Variational Bayes inference of the graphon (3/3)}
  ...
  \begin{itemize}
   ...
   \pt(K) \approx p(K|Y)
   $$ \\~
   \item A averaged estimate of $\gamma$ is given by \Refer{Latouche and R. (2015)}\nocite{LaR15}:
   $$
...
\frame{\frametitle{A first criterion for goodness-of-fit: Motifs frequency}
  ...
  \bigskip
  \ra Goodness of fit criterion based on motif frequencies? \Refer{Latouche and R. (2015)}\nocite{LaR15}.
...
\frame{\frametitle{Moments of motif counts}
  ...
  $$
  Estimates of $\Esp_W N(m)$ and $\Var_W N(m)$: see \Refer{Latouche and R. (2013)}\nocite{LaR13}.
