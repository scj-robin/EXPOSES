...
\frame{\frametitle{Bipartite network}
  ...
  \begin{tabular}{ll}
    \begin{tabular}{p{.45\textwidth}}
      \paragraph{Two types of nodes (='actors').}
      \begin{itemize}
      \item Mutualistic: plant-pollinator
      \item Antagonistic: host-parasite
      \end{itemize}
      \bigskip 
      \paragraph{Binary network:} \\
      edge = observed interaction \\
      \bigskip \bigskip 
      \onslide+<2>{\paragraph{Topological analysis:} \\
...
\frame{\frametitle{Bipartite network: notations}
  ...
  \begin{tabular}{ll}
    ...
    \begin{tabular}{p{.6\textwidth}}
      ...
      $$
      \item \emphase{adjacency matrix}: $m \times n$
      $$
...
\frame{\frametitle{Need for a null model}
  ...
  \begin{itemize}
   \item the \emphase{size} of the network: $m \times n$
  ...
  \begin{itemize}
    ...
    each pollinator $i$ has a specific propensity to interact (degree of generalism)
...
\frame{\frametitle{\BEDD model}
  ...
  \paragraph{Bipartite expected degree distribution (\BEDD) model:}  \\~
  \begin{itemize}
    \setlength{\itemsep}{1.15\baselineskip}
    \item $\rho =$ network density ($\in [0, 1]$)
    \item $g =$ top node degree imbalance ($g: [0, 1] \mapsto \Rbb^+, \quad \emphase{\int g = 1}$)
    \item $h =$ bottom node degree imbalance ($h: [0, 1] \mapsto \Rbb^+, \quad \emphase{\int h = 1}$)
  \end{itemize}
...
\frame{\frametitle{Properties of the \BEDD model}
  ...
  \begin{itemize}
    \setlength{\itemsep}{1\baselineskip}
    \item Pollinator degrees + plant degrees or, equivalently, 
    \item star (single edge, top, bottom) frequencies (see next)
  \end{itemize}
  ...
  \begin{tabular}{ll}
    \begin{tabular}{p{.4\textwidth}}
      ...
      \begin{itemize}
       ...
       \item Try the $r_s$ {\sl automorphisms} 
      \end{itemize}
      There are $c_s$ 'positions'
      $$
      c_s := 
...
% 
%   \paragraph{Occurrence probability $\overline{\phi}_s = \Pbb\{Y_{s\alpha} = 1\}$.} Consider the motif $s = \includegraphics[width=.06\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9}$, which involves
%   \begin{itemize}
%     \item 3 top nodes (with resp. latent $U_1$, $U_2$ and $U_3$) and
%     \item 2 bottom nodes (with resp. latent $V_1$ and $V_2$).
%   \end{itemize}
% 
...
%   \begin{align*}
%     \overline{\phi}_s 
%     & = \int \dots \int \left(\rho g(u_1) h(v_2)\right) \left(\rho g(u_2) h(v_2)\right) \left(\rho g(u_3) h(v_1)\right) \left(\rho g(u_3) h(v_2)\right) \d u_1 \d u_2 \d u_3 \d v_1 \d v_2 \\
%     & \onslide+<3->{= \rho^{-4} \left(\int \rho g(u_1) \d u_1\right) \left(\int \rho g(u_2) \d u_2\right) \left(\int \rho^2 g(u_3)^2 \d u_3\right) \\
%     & \qquad \left(\int \rho h(v_1) \d v_1\right) \left(\int \rho^3 h(v_2)^3 \d v_2\right)}  \\
%     & \onslide+<4->{= \rho^{-4} 
%       \quad \times \quad 
%       \phi\left(\includegraphics[width=.05\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9-top1}\right) 
%       \phi\left(\includegraphics[width=.05\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9-top1}\right) 
%       \phi\left(\includegraphics[width=.05\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9-top2}\right) 
%       \quad \times \quad 
%       \phi\left(\includegraphics[width=.05\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9-bottom1}\right) 
%       \phi\left(\includegraphics[width=.05\textwidth, trim=100 200 100 0]{\fignet/FigMotifsBEDD-motif9-bottom3}\right) }
%   \end{align*}
% 
%   \bigskip 
%   \onslide+<5->{\ra The probability of star motifs are the sufficient statistics of the \BEDD model.} 
% }

%==================================================================
\frame{\frametitle{Motif probability}

\paragraph{Occurrence probability $\overline{\phi}_s = \Pbb\{Y_{s\alpha} = 1\} =$} $\Pr\{\text{motif $s$ occurs at position $\alpha$}\}$

\bigskip \bigskip \pause
Under the B-EDD model:
\begin{align*}
    \overline{\phi}_s 
    & = \Esp_{(U_i), (V_j)}\left[\Pr\{\text{all edges from $s$ occur} \mid (U_i), (V_j)\}\right] \\
    & = \Esp_{(U_i), (V_j)}\left[\prod_{\text{edge in $s$}} \rho g(U_i) h(V_j)\right] \\
%     & = \int\dots\int \left( \prod_{\text{edges}} \rho g(u_i) h(v_j) \right)
%     \left(\prod_{i = \text{top node}} \d u_i\right)
%     \left(\prod_{j = \text{bottom node}} \d v_j\right) \\
    & = \rho^{- \text{nb edges}} 
    \prod_{i = \text{top node}} \underset{A_i}{\underbrace{\Esp_{U_i}\left[(\rho g(U_i))^{\text{degree($i$)}}\right]}}
    \prod_{j = \text{bottom node}} \underset{B_j}{\underbrace{\Esp_{V_j}\left[(\rho h(V_j))^{\text{degree($j$)}}\right]}} 
%     & = \rho^{- \text{nb edges}} 
%     \left(\prod_{i = \text{top node}} \int (\rho g(u_i))^{\text{degree of $i$}} \d u_i\right)
%     \left(\prod_{j = \text{bottom node}} \int (\rho h(v_j))^{\text{degree of $j$}} \d v_j\right) \\
%     & = \rho^{- \text{nb edges}} 
%     \left(\prod_{i = \text{top node}} (\text{degree($i$)-top star probability}) \right)
%     \left(\prod_{j = \text{bottom node}} (\text{degree($j$)-bottom star probability})\right)
\end{align*}
where
\begin{align*}
  A_i & = \text{occurrence probability for a top-star with degree($i$)}, \\
  B_j & = \text{occurrence probability for a bottom-star with degree($j$)}, \\
\end{align*}

{\ra The occurrence probabilities of star motifs are the sufficient statistics of the \BEDD model.} 

}

%==================================================================
\frame{\frametitle{Motif probability: An example}

  \paragraph{Occurrence probability $\overline{\phi}_s = \Pbb\{Y_{s\alpha} = 1\}$.} 
  \begin{align*}
...
\frame{\frametitle{Moments of the count}
  ...
  $$ 
  \ra Compute the respective expected count in the same way as for regular motifs \\ ~ \\
  \ra (No need to actually count super-motifs!) \\ ~
...
\frame{\frametitle{Distribution of the count}
  ...
  \begin{itemize}
    \item the (asymptotic) expected behavior (mean, variance, distribution) of any motif count 
...
%==================================================================
%==================================================================
\section{{\sl Application:} Network embedding}
\frame{\frametitle{Outline} \tableofcontents[currentsection]}
%==================================================================
...
\frame{\frametitle{Network embedding: Zackenberg's data \refer{SRO16}}
  ...
  \begin{tabular}{c|c|c|c}
    \onslide+<1->{Raw counts $N$} & 
    \onslide+<2->{Corrected stat. $T'$} & 
    \onslide+<3->{Choleski $T''$} & 
...
\frame{\frametitle{Goodness-of-fit (GOF) of the BEDD model}
  ...
  \bigskip \bigskip 
  \paragraph{Aim.} Test $H_0 : \{G \sim \BEDD\}$.

  \pause \bigskip \bigskip 
  \begin{tabular}{lc}
    \hspace{-.05\textwidth}
    \begin{tabular}{p{.5\textwidth}}
      \onslide+<2->{\paragraph{Raw statistic:} for motif $s$
      $$
      ...
      \onslide+<4>{\paragraph{Choleski transform:} accounts for the correlation between the counts
      $$
    ...
    \begin{tabular}{p{.4\textwidth}}
      \hspace{-.05\textwidth}
      \onslide+<2->{\paragraph{Zackenberg network:} dec 1996 \\}
      \begin{overprint}
        \onslide<2>
...
\frame{\frametitle{Testing degree imbalance}
  ...
  \begin{itemize}
    ...
    \emphase{H_0 = \{h = 1\}}
    $$
    \item For motif $s$, evaluate $\widehat{\Esp}_0(N_s) = \widehat{\Esp}_{\widehat{\rho}, \widehat{g}, \emphase{h_0}}(N_s)$ and $\widehat{\Var}_0(N_s) =  \widehat{\Var}_{\widehat{\rho}, \widehat{g}, \emphase{h_0}}(N_s)$ and use 
    $$
    W_s = (N_s - \widehat{\Esp}_0(N_s)) / \sqrt{\widehat{\Var}_0(N_s)} \underset{H_0}\longrightarrow\Ncal(0, 1)
    $$
  \end{itemize}
...
\frame{\frametitle{Comparing network imbalances}
  ...
  \begin{itemize}
    ...
    H_0 = \{g^A = g^B\}
    $$
    \item For motif $s$, evaluate $\widehat{\Esp}_{\widehat{\rho}^A, \emphase{\widehat{g}^B}, \widehat{h}^A}(N^A_s)$ and $\widehat{\Esp}_{\widehat{\rho}^B, \emphase{\widehat{g}^A}, \widehat{h}^B}(N^B_s)$ and use 
    $$
    W_s = \frac{(N_s^A - \widehat{\Esp}_0(N_s^A)) - (N_s^B - \widehat{\Esp}_0(N_s^B))}{\sqrt{\widehat{\Var}_0(N^A_s) + \widehat{\Var}_0(N^B_s)}}
    \underset{H_0}\longrightarrow\Ncal(0, 1)    
    $$ 
  \end{itemize}
...
%==================================================================
%==================================================================
\section{{\sl Application:} Contrast-based network distance}
\frame{\frametitle{Outline} \tableofcontents[currentsection]}
%==================================================================
\frame{\frametitle{Understanding the organisation of plant-pollinator networks}

  \bigskip
  On-going work with Natasha de Manincor (Mons, Belgium) \& Fran\c{c}ois Massol (soon in IEES, SU)

  \bigskip \bigskip 
  \paragraph{Aim.} Compare the organisation of plant-pollinator networks in space and time. \\
  \bigskip 
  \ra \BEDD translation: 
  $$
  \text{Compare the parameters $\rho$, \emphase{$g$} and $h$ in space and time}.
  $$
  ...
  \begin{itemize}
    ...
    \item 3 regions (Hauts-de-France, Normandie, Occitanie) $\times$ 2 sites / region
    \item 2 years (2016, 2017)
    \item 7 months (from april to october)
  \end{itemize}
  
  \bigskip
  \ra 83 networks, $\simeq 34$ pollinators ($\in [7; 80]$), $\simeq 13$ plants ($\in [3; 39]$)

}
  
%==================================================================
\frame{\frametitle{Contrast-based network distance}

  \bigskip \bigskip 
  \paragraph{Problems.} 
  \begin{enumerate}
    \setlength{\itemsep}{1.05\baselineskip}
    \item Poor power of network comparison tests for, say $H_0 = \{g^A = g^B\}$
    \item Which motif $s$ to determine the (contrast) test statistics $W_s$?
  \end{enumerate}

  \pause \bigskip \bigskip 
  \paragraph{Contrast-based distance.} 
  $$
  d_g(A, B) = \left(\sum_s W_s^2\right)^{1/2}
  $$
  where $W_s =$ test statistics based on motif $s$ for 
  $$
  H_0 = \{g^A = g^B\}.
  $$

  \bigskip 
  \paragraph{Rational.} The distance $d_g$
  \begin{enumerate}
    \setlength{\itemsep}{1.05\baselineskip}
    \item $d$ focuses on the difference in terms of pollinator imbalance and
    \item is (hopefully) insensitive to other factors: networks' dimensions ($m, n$), connectance ($\rho$), plant imbalance ($h$), \dots
  \end{enumerate}
...
\begin{frame}[fragile] \frametitle{Results}
  ...
  \paragraph{Distance-based analysis of variance.} 'Adonis' \refer{LeA99,McA01}
  \begin{itemize}
  ...
  {\small
  $$
%   Résultats Adonis vrais : All_NatashaBinNetList-stat1-adonis1:;;;;;
  \begin{tabular}{l|rrrrrl}
    & Df & SumOfSqs & $R2$ & $F$ & $Pr(>F)$ \\
    \hline
    Year & 1 & 2.24 & 0.0083 & 1.33 & 0.296 \\
    Month & 6 & 96.7 & 0.3589 & 9.57 & \emphase{0.001} \\
    Region & 2 & 12.7 & 0.0471 & 3.77 & \emphase{0.012} \\
    Year:Month & 6 & 4.89 & 0.0182 & 0.48 & 0.894 \\
    Year:Region & 2 & 7.1 & 0.0264 & 2.11 & 0.081 \\
    Month:Region & 12 & 45.57 & 0.1691 & 2.25 & \emphase{0.007} \\
    Year:Month:Region & 12 & 32.86 & 0.1219 & 1.63 & 0.054 \\
    \hline
    Residual & 40 & 67.36 & 0.25 & & \\
    Total & 81 & 269.42 & 1 & & \\
  \end{tabular}
  $$
% \begin{verbatim}
...
\begin{frame}[fragile] \frametitle{Yes but}
  ...
  $$
%   All_FakeBinNetList-sameColFALSE-sameRowFALSE-sameDensityFALSE-seed1-stat1-adonis1;;;;;
  \begin{tabular}{l|rrrrrl}
    & Df & SumOfSqs & $R2$ & $F$ & $Pr(>F)$ \\
    \hline
    Year & 1 & 1.5 & 0.0114 & 0.94 & 0.460 \\
    Month & 6 & 25.17 & 0.1914 & 2.61 & \emphase{0.011} \\
    Region & 2 & 0.69 & 0.0052 & 0.21 & 0.859 \\
    Year:Month & 6 & 7.46 & 0.0568 & 0.77 & 0.621 \\
    Year:Region & 2 & 2.52 & 0.0191 & 0.78 & 0.548 \\
    Month:Region & 12 & 12.55 & 0.0954 & 0.65 & 0.832 \\
    Year:Month:Region & 12 & 17.39 & 0.1322 & 0.9 & 0.585 \\
    \hline
...
\begin{frame}[fragile] \frametitle{A patch}
  ...
  \begin{overprint}
    \onslide<2>
    \paragraph{Simulated networks.}   {\small
    $$
  %   All_FakeBinNetList-sameColFALSE-sameRowFALSE-sameDensityFALSE-seed1-stat1-adonis2;;;;;
    \begin{tabular}{l|rrrrrl}
      & Df & SumOfSqs & $R2$ & $F$ & $Pr(>F)$ \\
      \hline
      insectNb & 1 & 10.55 & 0.0803 & 7.17 & \emphase{0.005} \\
      plantNb & 1 & 7.32 & 0.0556 & 4.97 & \emphase{0.017} \\
      Year & 1 & 0.77 & 0.0059 & 0.53 & 0.635 \\
      Month & 6 & 8.96 & 0.0681 & 01.01 & 0.457 \\
      Region & 2 & 4.39 & 0.0334 & 1.49 & 0.237 \\
      Year:Month & 6 & 8.54 & 0.0649 & 0.97 & 0.491 \\
      Year:Region & 2 & 2.17 & 0.0165 & 0.74 & 0.577 \\
      Month:Region & 12 & 13.99 & 0.1064 & 0.79 & 0.689 \\
      Year:Month:Region & 12 & 18.9 & 0.1437 & 01.07 & 0.427 \\
      \hline
...
% \end{verbatim} 
  ...
  $$
%   All_NatashaBinNetList-stat3-adonis2;;;;;
  \begin{tabular}{l|rrrrrl}
    & Df & SumOfSqs & $R2$ & $F$ & $Pr(>F)$ \\
    \hline
    insectNb & 1 & 69.9 & 0.2595 & 42.69 & \emphase{0.001} \\
    plantNb & 1 & 31.17 & 0.1157 & 19.04 & \emphase{0.001} \\
    Year & 1 & 2.66 & 0.0099 & 1.62 & 0.24 \\
    Month & 6 & 24.8 & 0.092 & 2.52 & \emphase{0.017} \\
    Region & 2 & 8.67 & 0.0322 & 2.65 & \emphase{0.035} \\
    Year:Month & 6 & 4.81 & 0.0179 & 0.49 & 0.875 \\
    Year:Region & 2 & 5.51 & 0.0204 & 1.68 & 0.198 \\
    Month:Region & 12 & 32.41 & 0.1203 & 1.65 & 0.058 \\
    Year:Month:Region & 12 & 27.26 & 0.1012 & 1.39 & 0.165 \\
    \hline
...
%====================================================================
\frame{ \frametitle{On-going}

  \paragraph{Not shown}
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item No clear difference for plants ($H_0 = \{h^A = h^B\}$)
    \item More significance differences with the combined contrast ($H_0 = \{g^A = g^B, h^A = h^B\}$)
  \end{itemize}
  ...
  \begin{itemize}
    \setlength{\itemsep}{.75\baselineskip}
    \item On-going simulations to assess the efficience of the correction patch
    \item Visualisation (multivariate dimensional scaling)
  \end{itemize}
...
%==================================================================
\frame{\frametitle{Bray-Curtis 'distance'}

  \paragraph{Bray-Curtis dissimilarity \refer{BrC57}.} Originaly designed to compare the species abundance vectors 
  $$
  N = (N_1, \dots N_p)
  $$ 
  observed in sites $A$ and $B$:
  $$
  d_{BC}(A, B) = 1 - \frac{2 \sum_{j=1}^p \min(N_j^A, N_j^B)}{\sum_{j=1}^p N_j^A + N_j^B}
  $$
  
  \begin{itemize}
  \item Does not satisfy the triangular inequality.
  \end{itemize}
