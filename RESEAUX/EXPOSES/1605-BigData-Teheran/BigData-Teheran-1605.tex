...
\date[Teheran, 2016]{BigData, May'16, Teheran}
...
\frame{\frametitle{Heterogeneous means ...} 
  ...
  \paragraph{... not homogeneous,} that is to say: different from an Erdös-Renyi (ER) graph.
  
  \bigskip \bigskip 
  \paragraph{Erdös-Renyi random graph $\Gcal(n, p)$:} Consider $n$ nodes, node pairs $1 \leq i < j \leq n$ are independently connected with same probability $p$:
  $$
...
\frame{\frametitle{Latent space models}
  ...
  \bigskip \pause
  \paragraph{We focus here on model-based approaches}, in contrast with, e.g.
  \begin{itemize}
...
\frame{\frametitle{Variational (Bayes) inference}
  ...
  \paragraph{Popular strategy:} minimize the Küllback-Leibler divergence between $\Pt$ and $P$:
  $$
...
\frame{\frametitle{First half summary}
  ...
  And also
  \begin{itemize}
   \item Theoretical justifications of these approximations exist for SBM: \refer{CDP12}, \refer{MaM15}
...
\frame{\frametitle{Variational Bayes model averaging}
  ...
  \bigskip \bigskip \pause
  \paragraph{Variational Bayes model averaging (VBMA).} The optimal\footnote{in terms of Küllback-Leibler divergence} approximation of $P(K|Y)$ satisfies \refer{VMR12}: 
  $$
...
\newcommand{\GOFplot}{/home/robin/Dropbox/vbemapp/gof2/code_gof/plot}
  ...
  \begin{tabular}{cc}
    ...
    \begin{overprint}
     \onslide<1>
     \includegraphics[width=.45\textwidth]{../FIGURES/treed0}
     \onslide<2>
     \includegraphics[width=.45\textwidth]{../FIGURES/treed3}
    \end{overprint}
    \end{tabular}
  \end{tabular}
...
\frame{ \frametitle{Blog network}
  ...
  \begin{tabular}{cc}
    ...
    \begin{overprint}
     \onslide<1>
     \includegraphics[width=.45\textwidth, clip=]{../FIGURES/blogd0}
     \onslide<2>
     \includegraphics[width=.45\textwidth]{../FIGURES/blogd3}
    \end{overprint}
    \end{tabular}
  \end{tabular}
