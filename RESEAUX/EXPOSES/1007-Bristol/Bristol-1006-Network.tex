...
%====================================================================
\title[Uncovering structure in biological networks]{Uncovering structure in
...
\date[Statworks'10, Bristol]{Statworks,
  June 2010, Bristol}
%====================================================================
...
\frame{\frametitle{Understanding network structure}
  \paragraph{Latent variable models} constitute a convenient way to capture
  the underlying structure of a network.

  \pause
  \paragraph{For binary graphs}, the general setting is the following 
  (\refer{BJR07}): \pause
  \begin{itemize}
  \item an \emphase{unknown latent variable $Z_i$} is associated with
    each node: 
    $$
...
\frame{ \frametitle{Maximum likelihood inference}
  ...
  \begin{itemize}
  \item $ P(\Xbf; \thetabf) = \sum_{\Zbf} P(\Xbf, \Zbf; \thetabf) $ is
    generally \emphase{not tractable}; \pause
  \item $ P(\Xbf, \Zbf; \thetabf) $ is often \emphase{much easier} to
    compute ... but {$\Zbf$ is unknown}.
  \end{itemize}
...
\frame{ \frametitle{Variational EM}
  ...
  \begin{itemize}
  \item \paragraph{If $P(\Zbf|\Xbf)$ can be calculated}: taking $Q(\Zbf) =
    ...
    Markov chains ... \pause%(\refer{DLR77,MaP00,CMR05}).
  \item \paragraph{If $P(\Zbf|\Xbf)$ can not be calculated}: the best
    lower bound of $\log P(\Xbf)$ is obtained for
    $$
...
%   $$
  ...
  $$
  which maximise $\log P(\Xbf)$ if $Q(\Zbf) = P(\Zbf|\Xbf)$, or its
  lower bound.
  }
...
%====================================================================
\frame{ \frametitle{Approximation of $P(\Zbf|\Xbf)$ for MixNet}
  \paragraph{Problem:}  
  We are looking for
  $$
  Q^* = \arg\min_{Q \in \Qcal} KL[Q(\Zbf); P(\Zbf|\Xbf)].
...
\frame{ \frametitle{Best factorisable approximation}
  ...
  \gamma_{k\ell}).
  $$
  can be improved via a \emphase{belief propagation (BP) algorithm}.  
...
\frame{ \frametitle{Variational Bayes inference} \pause
  ...
  unobserved and we want to retrieve $P(\Zbf, \thetabf|\Xbf)$.\pause
...
%==================================================================== 
\frame{ \frametitle{Variational Bayes (VB) EM} 
  
  The optimal $Q^*(\Zbf, \thetabf) = Q^*_Z(\Zbf)Q^*_\theta(\thetabf)$
  can be recovered by an EM-like algorithm (\refer{BeG03})
  \begin{eqnarray*}
    \text{\emphase{M step:}} \qquad \log Q_{\thetabf}(\thetabf) & = &
    \phi(\thetabf)' \left\{\Esp_{Q_Z}[u(\Xbf, \Zbf)] +
      \nubf\right\} + \cst \\ 
    \\
    \text{\emphase{'E' step:}} \qquad \log Q_Z(\Zbf) & = &
    \Esp_{Q_\theta}[\phi(\thetabf)]' u(\Xbf, \Zbf) + \cst
  \end{eqnarray*}

  \bigskip\pause
  \paragraph{Mean-field approximation.} For (binary) network mixture
  models, (see also \refer{LBA10}) we get 
  \begin{eqnarray*}
    \tau_{iq}^{\VBEM} &\propto&
    e^{\psi(\widetilde{n_k})-\psi\left(\sum_{l=1}^Q
        \widetilde{n}_\ell\right)} \prod_{j \neq i}^n \prod_{l=1}^Q
    e^{\tau_{j\ell}^{\VBEM} \left\{\psi(\widetilde{\zeta}_{k\ell}) -
        \psi(\widetilde{\eta}_{k\ell} + \widetilde{\zeta}_{k\ell}) +
        X_{ij}
        [\psi(\widetilde{\eta}_{k\ell})-\psi(\widetilde{\zeta}_{k\ell})]
      \right\})}    
  \end{eqnarray*}\pause

  A similar relation holds for valued graphs provided that
  \emphase{$f_{k\ell}$ belongs to the exponential family}.
  }
...
\frame{ \frametitle{Few more about inference} 
  ...
  \bigskip\bigskip\pause
  \paragraph{Model selection.} \refer{DPR08} propose to choose $K$ via
  $$
  ICL(K) = \Esp_{Q^*}[\log P(\Zbf, \Xbf)] - .5 \left\{(K-1)\log n + K^2
  \log[n(n-1)/2]\right\} 
  $$
  which is equal to the usual BIC, up to the \emphase{entropy term
    $\Hcal(Q^*)$} ... which is almost zero (see later).
...
\frame{ \frametitle{Valued network}
  ...
  may be related to exogenous covariates. Including such covariates
  enlightens \emphase{residual heterogeneity}.

  \bigskip\bigskip\pause
  \paragraph{Generalised linear model.} In the context of exponential
  family, covariates $\ybf$ can be accounted for via
  $$
  ...
  \bigskip\bigskip\pause
  VEM or VBEM inference can be performed as well: \\
  This only results in a \emphase{(slight) modification of the M-step}.
  }
...
\frame{ \frametitle{Simulation Study}
  2-group SBM with parameters:
  \begin{description}
  \item[Case 1:] $\pi=\left(\begin{array}{cc}0.6 &
        0.4\end{array}\right)$,  $\gamma=\left(\begin{array}{cc}0.8 & 0.2
        \\0.2 & 0.5\end{array}\right)$;
  \item[Case 2:] $\pi=\left(\begin{array}{cc}0.6 &
        0.4\end{array}\right)$, $\gamma=\left(\begin{array}{cc}0.8 & 0.2
  ...
  \bigskip \bigskip \pause
  \paragraph{Comparison of the 4 methods: {\VEM}, {\VBEM}, BP and EM}  on
  small graphs ($n = 18$: because of the computation time required by
  for \emphase{exact EM}).  
...
\frame{ \frametitle{Influence of the graph size}
  ...
  \textcolor{blue}{{\VBEM}: $+$} in case 2 (most difficult). \\
...
%==================================================================== 
\frame{ \frametitle{Concentration for $P(\Zbf|\Xbf)$} 
  
  \paragraph{Theorem for binary graphs:} Under conditions
  \begin{description}
  \item[C1:] $\forall (k \ne k') \; \; \; \exists l \in (1,K) \; :
    \; \gamma_{kl} \ne \gamma_{k'l}$ or $\gamma_{lk} \ne \gamma_{lk'}$;
  \item[C2:] $\exists a>0 \; : \;
    \min_{1>\gamma_{kl}>0}(\gamma_{kl},1-\gamma_{kl})>a$;
  \item[C3:] $\exists b>0 \; : \; \min_k  \pi_{k} >b$;
  \end{description}
...
\frame{ \frametitle{Acknowledgements}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.55\textwidth}}
      ...
      Recherche \\
      \\
      \emphase{NeMo} project ANR-08-BLAN-0304-01 
    \end{tabular}
  \end{tabular} \pause
