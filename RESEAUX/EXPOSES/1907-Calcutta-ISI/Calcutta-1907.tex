...
\frame{\frametitle{Species interaction and abundance data \refer{MRA19}} 
  ...
  \paragraph{Question:} Describe the interactions between a set of species 
  ...
  p_\theta(Y_i \mid x_i)
  $$
  so that to distinguish between direct and indirect interactions.
}
...
\frame{\frametitle{Generic problem} 
  ...
  \bigskip \bigskip
  \ra Amounts to assume that the distribution is {\sl faithful} to a graph $G$, which is sparse
}
...
\frame{\frametitle{Graphical models} 
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.3\textwidth}}
	 ...
	 \input{../FIGURES/SimpleGraphModelY}
...
\frame{\frametitle{Tree shaped graphical model} 
 ...
 \input{../FIGURES/SpanningTree1}
 \end{tabular}

    & 
\begin{tabular}{p{.18\textwidth}}
\input{../FIGURES/SpanningTree2}
 \end{tabular}

    & \begin{tabular}{p{.18\textwidth}}
\input{../FIGURES/NotSpanning} 
\end{tabular}

    & \begin{tabular}{p{.18\textwidth}}
\input{../FIGURES/NotTree}
 \end{tabular}
...
\frame{\frametitle{Maximum likelihood inference} 
  ...
  $$
  \log p(Y) = \sum_{(j, k) \in T} \sum_i \log \psi_{jk}(Y_{ij}, Y_{ik}) + \cst
  $$
  
  Suppose each $\psi_{jk}$ has a parametric form and that MLEs $\widehat{\psi}_{jk}$ is available, then finding
  $$
  \widehat{T} = \arg\max_{T \in \Tcal} \sum_{(j, k) \in T} \sum_i \log \widehat{\psi}_{jk}(Y_{ij}, Y_{ik})
  $$
  amounts to solve a \emphase{maximum spanning tree} problem \refer{ChL68}.
  ...
  \bigskip \pause
  \paragraph{Underlying assumption:} the dependency structure of $Y$ is {\sl indeed} tree-shaped 
}
...
\frame{\frametitle{Tree-based mixture} 
  ...
  \begin{itemize}
    ...
    & \propto \; B^{-1} \sum_{T \in \Tcal} \prod_{(j, k) \in T} \beta_{jk} \; \psi_{jk}(Y_j, Y_k)
    \end{align*} 
  \end{itemize}
...
\frame{\frametitle{Matrix-tree theorem} 
  ...
  $$
  \ra \refer{Kir07}: all $[\Delta(W^{ab})]^{ab}$ can be computed at the price of one matrix inversion: \emphase{$O(p^3)$}
}
...
%         \hline
        $2.1\%$ & $3.5\%$ \\
        \input{../FIGURES/FigTreeAveraging-p4-tree1-seed2} &
        \input{../FIGURES/FigTreeAveraging-p4-tree2-seed2} \\
        \hline
        $34.1\%$ & $15.6\%$ \\
        \input{../FIGURES/FigTreeAveraging-p4-tree3-seed2} &
        \input{../FIGURES/FigTreeAveraging-p4-tree4-seed2} \\
        \hline
        $ < .1\%$ & \\ 
        \input{../FIGURES/FigTreeAveraging-p4-tree5-seed2} &
        \dots \\
      \end{tabular}
    \end{tabular}
    & 
    ...
    \begin{tabular}{p{.5\textwidth}}
      ...
      \begin{center}
      \input{../FIGURES/FigTreeAveraging-p4-avgtree-seed2} 
      \end{center}
      
      \bigskip \bigskip \bigskip \pause
      \paragraph{Infered network.} $\widehat{G}$
      \begin{center}
      \input{../FIGURES/FigTreeAveraging-p4-graph-seed2}
      \end{center}
...
\frame{\frametitle{Fish populations in the Fatala River estuary} 
  ...
  Not that many models for multivariate counts \refer{IYA17} \\
   ...
   \begin{align*}
      ...
      \{Y_{ij}\}_{ij} \text{ indep. } \mid \{Z_i\}_i, \quad Y_{ij} & \sim \Pcal\left(\exp(x_i^\intercal \gamma + Z_{ij})\right) 
   \end{align*}
   denoted as
   $$
...
\frame{\frametitle{Mixture of tree-shaped PLN models} 
  ...
  \begin{itemize}
   ...
   \ra fit a tree-shaped mixture using the EM-tree \\
...
\frame{\frametitle{Comparison with gCoda and SpiecEasy} 
  ...
  \begin{center}
  \includegraphics[width=.8\textwidth]{../FIGURES/MRA19-Fig6top} \\
  \includegraphics[width=.8\textwidth]{../FIGURES/MRA19-Fig6bottom} 
  \end{center}
...
\frame{\frametitle{Results for the Fatala river} 
  ...
  $$
  \includegraphics[width=.9\textwidth]{../FIGURES/MRA19-Fig8} 
  $$
...
\frame{\frametitle{A tree-shaped path} 
  ...
  $$
  \includegraphics[width=.6\textwidth]{../FIGURES/BSR19-Fig1}
  $$
...
\frame{\frametitle{An easy situation} 
  ...
  \begin{tabular}{lll}
    ...
    &
    \begin{tabular}{l}
      \includegraphics[width=.5\textwidth]{../FIGURES/BSR19-Fig2p}
    \end{tabular}
   &
   \begin{tabular}{p{.5\textwidth}}
...
\frame{\frametitle{Illustration} 
  ...
  \begin{tabular}{cc}
    \begin{tabular}{p{.4\textwidth}}
      \paragraph{Seed exchange network:} how farmers exchange seeds from one season to another. \\
...
\frame{\frametitle{Conclusion} 
  ...
  \begin{itemize}
    ...
    \item Numerical issues arise for large $p$ or $n$ (use tempering \refer{ScR17}?)
  \end{itemize}
}
