...
\rfoot{\begin{rotate}{90}{
      \hspace{1cm} \tiny S. Robin: Uncovering structure in biological
      networks   
      }\end{rotate}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Uncovering structure: A simple example}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \newpage
% \section{Heterogeneous random graphs}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{lcccc}
  %\vspace{-3cm}
  \subsection{Bernoulli case} & Network & $Q$ & $(\alpha_q)$ & $(\pi_{q
    \ell})$ \\ 
%   & Cluster. coef. \\
  \hline
  \begin{tabular}{p{2cm}} Random \\(Erdös) \end{tabular}
...
%   & $p$ \\
  ...
  \end{tabular}
  & 2 & $\left( \displaystyle{\textred{\frac12} \quad
  \textblue{\frac12}} \right)$  
...
%   & $\displaystyle{\frac{(a^2+b^2)^2}{(a+b)^2}}$ \\
  ...
  & \begin{tabular}{c} \epsfig{file = ../figures/FigNetworks-Star-Col.eps,
      width=3.6cm, clip=, angle=270}
  \end{tabular}
  & 4 & $\left( \displaystyle{\textblue{\frac4{12}} \quad
  \textred{\frac1{12}} \quad \textgreen{\frac1{12}} \quad \frac4{12}}
  \right)$   
...
\item \vspace{-0.5cm} $\RX$ is some distribution for $\Zbf$ (depending
  on $ \Xbf$).
\end{itemize}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \newpage
\paragraph{$\RX$ approximates the conditional distribution
  $\Pro(\Zbf|\Xbf)$.} We want it to be 
\begin{itemize}
...
The $\tau_i$'s are interpreted as \emphase{approximate posterior
  probabilities: $\tau_{iq} \simeq \Pro\{Z_i = q | \Xbf\}$}.
...
Denoting $\log \Pro(\Xbf, \Zbf | \gammabf,m_Q)$ the log-likelihood of
model $m_Q$ with $Q$ classes, we get
$$
...
where
\begin{itemize}
\item \vspace{-0.5cm} $P_Q$ = number of parameters in $\thetabf$ is
  penalised by the number of edges: $n(n-1)$;
\item \vspace{-0.5cm} $Q-1$ = number of proportions is penalised
  by the number of nodes: $n$.
\end{itemize}

\noindent $\widehat{\Zbf}$ can be replaced by $\widehat{\taubf}$ or by the
...
\paragraph{Dataset.}
\begin{itemize}
\item \vspace{-0.5cm} The network is made of 605 reactions (nodes) and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\chapter{Null model for heterogeneous networks}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{cc}
  \begin{tabular}{p{15cm}}
    \section{Looking for over-represented motifs} \\
    \\
    \paragraph{Strategy proposed by \refer{\sl Shen-Orr \& al, 02}}
    for  {\sl E. coli} transcriptional network. 
    \begin{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Direct computation using heterogenous models}

\noindent \hspace{-1cm}
\begin{tabular}{cc}
  \begin{tabular}{p{11cm}}
    \paragraph{Exact moments.} For several heterogeneous models
    (mixture, EDD), we can calculate the exact formula for the mean $\Esp N$
...
\item \vspace{-0.5cm} Once fitted to a given network, the mixture
  model allows to detect unexpetedly frequent motifs in
  biological (binary) networks.
...
\item \vspace{-0.5cm} The variational approach does not provide any
  ...
  $\rightarrow$ A variational Bayes approach would provide the
...
\paragraph{Articles:} 
\begin{itemize}
\item J.-J. Daudin, F. Picard, S. Robin. {\sl A mixture model for
    random graphs}. Stat. Comput., 18(2):173--83, Jun 2008.
\item F. Picard, J.-J. Daudin, M. Koskas, Schbath S., and Robin S.
  {\sl Assessing the exceptionality of network motifs.} J. Comp.
    Biol., 15(1):1--20, 2008
\end{itemize}

\paragraph{Research report:} 
\begin{itemize}
\item M. Mariadassou, S. Robin, {\sl Uncovering latent structure in
    valued graphs: a variational approach.} SSB Tech. Rep., 10,
  October 2007 \\
