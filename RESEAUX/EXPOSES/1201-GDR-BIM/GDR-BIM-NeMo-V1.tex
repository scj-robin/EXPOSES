...
\title[Some Statistics for Biological Networks]{Some Statistical
  Aspects of Biological Networks Analysis}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.45\textwidth}}
      ...
      \epsfig{file=../FIGURES/GGM-Corr-3.eps, clip=,
        width=1\textwidth} \Pause
      % \end{overprint}
      
      \vspace{-5cm} The elements of $\Sigmabf$ (and
...
\frame{ \frametitle{Regularization via penalization} 
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.45\textwidth}}
      \onslide+<1->{
        \paragraph{Regression setup} (e.g. GWAS): 
        \begin{itemize}
        \item $Y_i =$ phenotypes ($i = 1\dots n$),
        \item $\xbf_i = (x_{ik}) =$ SNP ($k = 1\dots p$), 
        \end{itemize}
        $$
        Y_i = \xbf_i \thetabf + E_i
        $$
      }
      
      \onslide+<2->{
        \bigskip
        \paragraph{Because $p \gg n$,} no solution for least-square
        regression 
        $$
        \widehat{\thetabf} = \arg\min_{\thetabf} \| \Ybf - \Xbf \thetabf \|^².
        $$
        \vspace{2cm}~
      }
    \end{tabular}
    ...
    \begin{tabular}{p{.55\textwidth}}
      \onslide+<3->{
        \paragraph{Regularized problems.}
        Geometric view. \\
      }
      ...
      \begin{overprint}
        ...
        $ ~\qquad \min_{\thetabf} \| \Ybf - \Xbf \thetabf \|^² +
        \lambda \|\thetabf\|_2^2 $  \\~\\
...
\frame{ \frametitle{Stochastic block-model for interaction graphs} 
  ...
  \begin{itemize}
  \item Nodes $i = 1\dots n$ are spread into $Q$ groups with proportions
    \emphase{$\alpha_1, \dots \alpha_Q$},
  ...
  \end{itemize}
  $$
  \emphase{\pi_{q\ell}} = \Pr\{X_{ij} = 1 | Z_i=q, Z_j = \ell\}.
  $$
}
...
%====================================================================
  ...
  \end{enumerate}\Pause
  In the case of graph model, \emphase{$P(\Zbf|\Xbf)$ is intractable}.
...
\frame{ \frametitle{A lot of statistical issues}
  ...
  \paragraph{Variational inference.} Including the choice of $Q$:
  \begin{itemize}
  ...
  \begin{tabular}{cc}
    ...
    \begin{tabular}{p{.5\textwidth}}
      \onslide+<2->{
        ...
        $$
        Z_i \sim \Mcal(1; \alpha_1, \dots, \alpha_Q)
        $$}
      \onslide+<3->{
        with $Q$ independent drawings
        $$
        Z_{iq} \sim \Bcal(\alpha_q)
        $$
...
\frame{ \frametitle{What motifs?}
  ...
  \paragraph{Local patterns} constitute {functional modules} or basic
  ...
  to
  \begin{enumerate}
  \item \Pause Give a precise (topological) definition of a motif $\mbf$;
  \item \Pause Be able to count (efficiently) motif occurrences in a given
    network;
  \item \Pause Define a random graph model as a null model $G_0$;
  \item \Pause Know the (approximate) distribution of the number occurrences
    under the null model $N(\mbf, G_0)$
  \end{enumerate}
  \Pause to compute the $p$-value
  $$
...
%==================================================================== 
\frame{\frametitle{Efficient algorithms for counting occurrences}

  \paragraph{Coloured motifs.} Already done by \refer{LFS06}.
  
  \bigskip\Pause
  \paragraph{Topological motifs.} Clever, pruned backtracking.
...
\frame{ \frametitle{Tools}
  ...
  \paragraph{Counting coloured motifs in metabolic networks.}
  \begin{itemize}
  \item Stand-alone software MOTUS
  \end{itemize}
  

  \bigskip\Pause
  \paragraph{Exceptional topological motifs in interaction networks.}
  \begin{itemize}
  \item R-package NEMO
  \end{itemize}

  \bigskip\Pause
  \paragraph{An local definition of exceptionality.} Locally
  over-represented motif, given the occurrence of a sub-motif:
  $$
  \epsfig{file = ../FIGURES/LocalMotif.eps, scale=.5, clip=}
  $$
  \begin{itemize}
  \item   R-package PALOMA (\refer{Bir09b})
  \end{itemize}
  }

%====================================================================
\section*{In progress / Future Works}
\frame{ \frametitle{Future} 
