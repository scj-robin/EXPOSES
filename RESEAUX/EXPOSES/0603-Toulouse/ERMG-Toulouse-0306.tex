...
\paragraph{Erdos-Rényi (ER) model.} $\{X_{ij}\}$ i.i.d.,  $\Pr\{ i
      \leftrightarrow j\} = \pi$ \\
$$
...
\paragraph{Erdos-Rényi model.} The popular Erdos-Rényi fits
...
Denote $\Xcal$ the set of random vertices and $\Zcal$ the set of labels.

The aim is to \paragraph{maximise the log-likelihood $\Lcal(\Xcal)$}
which is not calculable, partly because $\Pr(\Zcal\;|\;\Xcal)$ is
intractable.  We actually maximise
$$
...
$\Hcal(R_{\Xcal}) = -\sum_{\Zcal} R_{\Xcal}(\Zcal) \ln(\Zcal)$.
...
$$
\paragraph{Algorithm.} We alternately optimize $\Jcal(R_{\Xcal})$ in
$R_{\Xcal}$ and in the parameter estimates ($\widehat{\alpha},
\widehat{\pi}$), so the proposed algorithm converges.
...
where
\begin{itemize}
\item \vspace{-0.5cm} the $\taubf_i$ are variational parameters to be
  optimized;
\item \vspace{-0.5cm} $b(c; n, \pi)$ denotes the binomial
  ...
  \widehat{\tau}_{j\ell}$;  
\item \vspace{-0.5cm} $\widehat{\tau}_{j\ell}$ turns out to be
  approximate conditional expectations: $\widehat{\tau}_{j\ell} =
  \Esp_{R_{\Xcal}} Z_{j\ell}$;
\item \vspace{-0.5cm} $\widehat{\tau}_{j\ell}$ must satisfy the fix
  point relation: $ \widehat{\tau}_{j\ell} = \Pr\{Z_{iq} = 1 \;|\;
  \Xcal, \widehat{\Zcal}^i\}$. %\\
  %The $\widehat{\tau}_{j\ell}$ are obtained by iterating it.
\end{itemize}
...
\item \vspace{-0.5cm} Study the statistical properties of the estimates;
