...
%\nexcommand{\refer}[1]{\textcolor{blue}{\sl \cite{#1}}}
...
\date[SMA'11, Fréjus]{Statistique Mathématique \& Application,
  Septembre 2011, Fréjus}
...
%--------------------------------------------------------------------
\subsection{A mixture model for networks}
\frame{\frametitle{A mixture model for networks}

  \hspace{-.5cm}
  \begin{tabular}{p{.1\textwidth}p{.35\textwidth}p{.45\textwidth}}
    \emphase{Data} & Nodes & $i = 1, ... n$ \\
    \\
    & Edges & $X_{ij} = \Ibb\{i \sim j\}$ \\
    \\ \pause \\
    \emphase{Model} & Unknown node labels $\{Z_i\}$ & i.i.d. $\sim \Mcal(1;
    \pibf)$ \\
    \\
    & Edges $\{X_{ij}\}$ indep. $| \{Z_i\}$ 
    & $\Pr\{X_{ij} = 1 | Z_i = k, Z_j = \ell\}  = \gamma_{k\ell} $ \\
    \\ \pause \\
    \emphase{More} & {Valued graphs} & $X_{ij} \in \{0, 1\}$,
    $\{0, \rightarrow, \leftarrow, \leftrightarrow\}$, \\ 
    & (e.g. oriented graphs):
    & ${\mathbb N}$, ${\mathbb R}$, ...; \\ 
    \\
    & {Other distributions:} & Bernoulli, multinomial, Poisson,
    Gaussian, ....
  \end{tabular}
...
%     {Data:} 
%     & \emphase{Nodes $i =$} $1, ... n$; \\
%     & \emphase{Edges $X_{ij} =$} $\Ibb\{i \sim j\}$, observed. \\
%     \\
%     {Model:} 
%     & \emphase{Unobserved node labels $Z_i$:} independent, \\
%     &
%     \medskip
%     \centerline{$
%       \Pr\{Z_i = k\} = \pi_k
% % \{Z_i\}_i \mbox{ i.i.d.}, \qquad Z_i \sim \Mcal\left(1; \pibf =
% %         [\pi_1, \dots \pi_K]\right)
%       $} 
%     \\ 
%     & \emphase{Observed edge values:} $\{X_{ij}\}_{i,
%       j}$ indep. cond. on $\{Z_i\}$: \\
%     &
%     \medskip
%     \centerline{$
...
%     $} \\
%     {Generalisations:} 
%     & \emphase{Valued graphs} (including oriented graphs): \\
%     &
%     \medskip
%     \centerline{$
%     X_{ij} \in \{0, 1\}, \{0, \rightarrow, \leftarrow,
%     \leftrightarrow\}, {\mathbb N}, {\mathbb R}, ...;
%     $} \\ 
%     & \emphase{Other distributions:} Bernoulli, multinomial, Poisson,
%     Gaussian, ....
%   \end{tabular}
  }
...
%           underlined. 
          \paragraph{Aim.} \\
          Estimates: $\widehat{\thetabf} = (\widehat{\pibf},
          \widehat{\gammabf})$, \\
          Classif.: $\Pr\{Z_i=  k \;|\; \Xbf\}$.
          }
      \end{tabular}
      & 
...
\frame{ \frametitle{Maximum likelihood inference}
  ...
  \Esp_{P(\cdot|\Xbf)}[\log P(\Zbf|\Xbf)],
%   \log P(\Xbf) = \Esp[\log P(\Xbf, \Zbf) | \Xbf] -
%   \Esp[\log   P(\Zbf|\Xbf) | \Xbf], 
  $$
  \pause
  ...
  \item[M-step:] maximisation of $\Esp_{P(\cdot|\Xbf)}[\log P(\Xbf,
    \Zbf; \thetabf)]$ in $\thetabf$. 
...
%     \Xbf]$ in $\thetabf$. 
  ...
  \begin{itemize}
  \item In presence of a \emphase{complex dependency structure}, the
    calculation of the conditional distribution $P(\Zbf|\Xbf)$ may not
    be possible.
  \item  Variational inference is based on an \emphase{approximation of
    $P(\Zbf|\Xbf)$} by some 'optimal' distribution $Q(\Zbf)$.
  \end{itemize}
...
\frame{ \frametitle{Variational approximation}
  ...
  \begin{eqnarray*}
    ...
    %\pause \\
    & = & \Esp_{\emphase{Q}}[\log \emphase{P(\Xbf, \Zbf)}] + \Hcal(Q) 
  \end{eqnarray*}
  ...
  $$
  \log P(\Xbf) = \Esp_{\emphase{{P(\cdot|\Xbf)}}}[\log P(\Xbf, \Zbf)] +
  \Hcal[P(\cdot|\Xbf)]. 
  $$

  \bigskip\pause
  \paragraph{Pseudo E-step of $\VEM$:}
...
%     calculated.
  \item We restrict ourselves to the set of \emphase{factorisable
      distributions}:
    $$
    \Qcal = \left\{Q: Q(\Zbf) = \emphase{\prod_i} Q_i(Z_i) = \prod_i \prod_k
      \tau_{ik}^{Z_{ik}}\right\}, 
    \quad
...
\frame{ \frametitle{Variational Bayes inference} 
  ...
  \bigskip\pause
  \paragraph{$\VBEM$ algorithm:} In the exponential family / conjugate
  prior context
  $$
...
%--------------------------------------------------------------------
\section{Quality of the variational inference for SBM}
%--------------------------------------------------------------------
\frame{ \frametitle{Quality of the variational inference for SBM}}

%--------------------------------------------------------------------
\frame{ \frametitle{Properties of variational inference}

  Still few is known about the properties of both $\VEM$ and $\VBEM$
  inference. \\ 
  
  \bigskip\pause
  \paragraph{$\VEM$ inference.} 
  \begin{itemize}
  \item \refer{GuB05} prove that the $\VEM$ algorithm converges to a
    \emphase{different optimum than ML} in the general case, except
    for \emphase{'degenerated' models}.
  \item But random graphs have a specific asymptotic framework:
    \emphase{$n^2$ data, $'p' = n$ 'variables'} per individual.
  \item Mean field approximation is asymptotically exact for some
    models with infinite range dependency \refer{OpW01}.
  \end{itemize} 

  \pause
  \paragraph{$\VBEM$ inference.}   
  \begin{itemize}
  \item Consistency is proved for \emphase{some incomplete data models}
    \refer{McT09}.
  \item In practice, $\VBEM$ often under-estimates the posterior
    variances.
  \end{itemize}
  }
...
\frame{ \frametitle{Variational Bayes approximation: Simulation Study}
  ...
  \begin{itemize}
  \item 2-group binary SBM
  \item 500 graphs are simulated for each scenario and each graph
    size.
%   \item Comparison of \textcolor{red}{{\VEM} ($\bullet$)} and
%   \textcolor{blue}{{\VBEM} ($+$)}
  \end{itemize}
...
\frame{ \frametitle{Concentration of $P(\Zbf|\Xbf)$ for binary graphs} 
  ...
  Let $g_\Xbf$ denote the (random) function
  $$
  g_\Xbf(\zbf) := \Pr\{\Zbf=\zbf|\Xbf\} = \frac1C \prod_i
...
%  This function is random, as it depends on $\Xbf$.
  ...
  \pause
  \paragraph{Theorem \refer{CDP11}.} Under 
  identifiability conditions and if $\forall k, \ell: 0 < a <
  \gamma_{k\ell} < 1-a, 0 < b < \pi_k$, then we have
  $$
  \forall t>0, \qquad \Pr\left\{ \left. \frac{\sum_{\zbf \neq
          \zbf^*} g_\Xbf(\zbf)}{g_\Xbf(\zbf^*)} > t \right| \Zbf =
    \zbf^* \right\} = \Ocal(n e^{-\kappa(t) n}).
  $$
...
%--------------------------------------------------------------------
\frame{ \frametitle{Why does variational inference work for SMB?}  
  
  \refer{CDP11} suggests that $P(Z|X)$ concentrates around the true
  value $z^*$, i.e.
  $$
  P(Z|X) \underset{n \rightarrow \infty}{\longrightarrow} \delta_{z^*}(Z).
  $$
  This is due to the specific asymptotic framework where $'p' = n$.

  \bigskip
  \paragraph{Intuition:} 
  \begin{enumerate}[($i$)]
  \item The limit distribution $\delta_{z^*}(Z)$ belongs to the
    distribution class $\Qcal$ \\
    ... over which both {\VEM} and {\VBEM} approximations achieve
    maximisation, so it is reached;
  \item As for \VBEM, the joint conditional distribution $P(\theta,
    Z|X) = P(\theta|Z, X) P(Z|X)$ tends to $P(\theta|Z, X)
    \delta_{z^*}(Z)$.  \\
    Again $P(\theta|Z, X)$ belongs to the distribution class of
    {\VBEM}, so it is also reached;
  \end{enumerate}
  And the variational approximation for SBM tends to be ... exact.
  }

%-------------------------------------------------------------------- 
\frame{ \frametitle{Few more about inference} 
  \paragraph{Identifiability.} Even for binary edges, SBM is
...
%     heterogeneity}.
  ...
  nice set-up to account for covariates.
