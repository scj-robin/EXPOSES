...
\noindent Denote $\Xcal$ the set of the observed edges and $\Zcal$ the
set of the unobserved labels. 

We would like to \paragraph{maximize the log-likelihood
  $\Lcal(\Xcal)$} which is not calculable (need to sum over all
possible $\Zcal$).  We actually maximize
$$
...
\noindent E-M is very popular to fit mixture models. It requires to
calculate the conditional distribution \paragraph{$\Pr\{\Zcal \;|\;
  \Xcal\}$} which is intractable.
$$
...
\noindent We propose a heuristic penalized likelihood criterion inspired from
ICL ({\sl Biernacki \& al., 2000}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Conclusions on the ERMG model}
