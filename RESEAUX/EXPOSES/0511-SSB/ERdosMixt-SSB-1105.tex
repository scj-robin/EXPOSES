...
\paragraph{Notation and definition.} Given a set of $n$ edges ($i = 1..n$),
$X_{ij}$ indicates the presence/absence of a (non oriented) vertex
...
\noindent\begin{tabular}{ll}
  Degree (connectivity) of the edges: & ${K_i = \sum_{j
      \neq i} X_{ij}}$ \\
  \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\chapter{Erdös-Rényi mixture for graphs (ERMG)} \label{Sec:ErdosMixture}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\paragraph{Clustering coefficient:}  $c = \Pr\{\nabla \cap \Vsf\} /
\Pr\{\Vsf\} = \Pr\{\nabla\} / \Pr\{\Vsf\}$. 
...
\paragraph{Additional conditioning.} $\theta_{ijq\ell}$ turns out to
be difficult to evaluate.

We propose to approximate it by
$$
...
Completed Likelihood (ICL, Biernacki et al., 2000) which applies the same penalty
as BIC to the complete-data log-likelihood (only available here).  

$\Qcal(\Xcal)$ is the sum of
$$
\begin{tabular}{p{11cm}p{13cm}}
  $\displaystyle{\sum_i \sum_q \tau_{iq} \log \alpha_q}$ & 
  which deals with $Q$ proportions $\alpha_q$s and involves $n$ data
  \\ 
  \\
