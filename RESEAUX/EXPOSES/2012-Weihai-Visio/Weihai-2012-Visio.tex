...
%====================================================================
\title[Tree-based mixtures]{Tree-based mixture distributions: \\ \medskip
...
\date[Weihai, Dec.'20]{Shandong University, Weihai, Dec. 2020}
...
%====================================================================
\frame{\frametitle{Species interaction and abundance data \refer{MRA20}} 
...
\frame{\frametitle{Graphical models} 
  ...
  $$
  \ra Markov property: $G$ encodes the conditional dependence structure of $p$:
  $$
...
\input{\fignet/NotTree}
 \end{tabular}
  ...
  $$
  (because the cliques are only edges) 
...
\frame{\frametitle{Maximum likelihood inference} 
  ...
  $$
  amounts to solve a \emphase{maximum spanning tree} problem (Chow \& Liu algorithm: \refer{ChL68})\footnote{Actually $\psi_{jk}(Y_{ij}, Y_{ik}) = p(Y_{ij}, Y_{ik}) / \left(p(Y_{ij}) p(Y_{ik})\right)$, so edge weight $\approx$ mutual information}
...
\frame{\frametitle{Matrix-tree theorem} 
  ...
  $$
  \ra Cost = computation of a determinant: \emphase{$O(p^3)$}
...
\frame{\frametitle{EM algorithm} 
  ...
  $$
  so the objective function of EM is\footnote{Central quantity in EM: $\Esp(\log p_\theta(Y, T) \mid Y)$}
  $$
  \sum_{j, k} \emphase{\Pr\{(j, k) \in T \mid Y\}}\left( \log \beta_{jk} + \sum_i \log \psi_{jk}(Y_{ij}, Y_{ik}) \right) - \log \emphase{B}
  $$
  
  \bigskip \bigskip \pause
  \paragraph{'EM-tree' algorithm.} \refer{MRA20}
  \begin{itemize}
...
\frame{\frametitle{Mixture of tree-shaped PLN models} 
  ...
  \bigskip \bigskip \pause
  \paragraph{Two-step procedure \refer{MRA20}.}
  \begin{itemize}
   \item \pause Observed layer: \\
   \ra fit a PLN model using a VEM algorithm (\url{PLNmodels} package: \refer{CMR18a}) \\
...
\frame{\frametitle{Conclusion} 
  ...
  \begin{itemize}
   ...
   \item Tree-shaped mixtures can be used to infer missing nodes \refer{RAR18,MRA20b}
  \end{itemize}
...
%====================================================================
\frame{\frametitle{Missing actors \refer{MRA20b}} 

  \paragraph{Inference of ecological network:} Same setting as \refer{MRA20} but assuming $p+r$ hidden variables: $Z = (Z_O, Z_H)$. \\
  \ra Variational EM, approximating $p_\theta(T, Z_0, Z_H \mid Y)$ 
  
  \pause \bigskip \bigskip
  \paragraph{Model without covariates:} Infers $r = 1$ missing actor, high correlated with the temperature
  
  $$
  \begin{array}{c|c}
    r = 0 \qquad \qquad \qquad r = 1 & Z_H = f(\text{Temp.}) \\
%     \hline
    \includegraphics[width=.45\textwidth]{\fignet/MRA21-Fig6}
    &
    \includegraphics[width=.25\textwidth]{\fignet/MRA21-Fig7}
  \end{array}
  $$
