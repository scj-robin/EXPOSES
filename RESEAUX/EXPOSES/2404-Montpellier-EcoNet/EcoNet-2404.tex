...
\frame{\frametitle{Moments of the count}
  ...
  \item \emphase{Variance:} Same game, requires to evaluate $\displaystyle{\Esp_{\BEDD}(N_s ^2) = \Esp_{\BEDD}\left(\sum_\alpha Y_{s\alpha} \right)^2}$ \\
...
\frame{\frametitle{Distribution of the count}
  ...
  \paragraph{Asymptotic normality for non-star motifs.} Under \BEDD (and sparsity conditions \refer{OLR22}):
  $$
...
\frame{\frametitle{Network embedding}
  \todo{aim of embedding, udins motif, accounting for their covariance}
}
